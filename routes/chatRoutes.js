...
router.post('/', async (req, res) => {
    try {
        ...
        await Promise.all(participants.map(async (participantId) => {
            await index_1.User.findOneAndUpdate({ id: participantId }, {
                $push: {
                    recentActivities: {
...
router.post('/send', async (req, res) => {
    try {
        ...
        index_1.User.findOneAndUpdate({ id: from }, {
            $push: {
                recentActivities: {
        ...
        try {
            ...
            const sender = await index_1.User.findOne({ id: from }).select('name');
    ...
    try {
        ...
        try {
            ...
            if (receiverIds.length > 0) {
                const sender = await index_1.User.findOne({ id: senderId }).select('name');
                const senderName = sender?.name || 'Alguém';
                const preview = messageType === 'image' ? '[Imagem]' : content;
                for (const receiverId of receiverIds) {
                    await NotificationService.notifyNewMessage(io, receiverId, senderId, senderName, preview, id);
                }
            }
        }
        catch (notifErr) {
