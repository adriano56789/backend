...
router.get('/users/:id/status', async (req, res) => {
    try {
        ...
        await models_1.User.findOneAndUpdate({ id: id }, {
            $push: {
                recentActivities: {
        ...
        let userStatus = await models_1.UserStatus.findOne({ userId: id });
        console.log(`[DEBUG] Status encontrado:`, userStatus ? 'SIM' : 'NÃO');
        if (!userStatus) {
            // Criar status padrão se não existir com upsert automático
            userStatus = await models_1.UserStatus.findOneAndUpdate({ userId: id }, { $setOnInsert: { userId: id, isOnline: false, lastSeen: new Date() } }, { upsert: true, returnDocument: 'after' });
        }
        res.json({
...
router.post('/users/:id/online', async (req, res) => {
    try {
        const { id } = req.params;
        const now = new Date();
        console.log(`🟢 [STATUS] Marcando usuário ${id} como ONLINE`);
        // 1. Atualizar modelo UserStatus
        await models_1.UserStatus.findOneAndUpdate({ userId: id }, { $set: { isOnline: true, lastSeen: now } }, { upsert: true, returnDocument: 'after' });
        await models_1.User.findOneAndUpdate({ id: id }, {
...
router.post('/users/:id/offline', async (req, res) => {
    try {
        const { id } = req.params;
        const now = new Date();
        console.log(`🔴 [STATUS] Marcando usuário ${id} como OFFLINE`);
        // 1. Atualizar modelo UserStatus
        await models_1.UserStatus.findOneAndUpdate({ userId: id }, { $set: { isOnline: false, lastSeen: now } }, { upsert: true, returnDocument: 'after' });
        await models_1.User.findOneAndUpdate({ id: id }, {
...
router.put('/users/:id/status', async (req, res) => {
    try {
        ...
        const userStatus = await models_1.UserStatus.findOneAndUpdate({ userId: id }, { $set: updateData }, { upsert: true, returnDocument: 'after' });
        res.json({
            success: true,
...
router.get('/online', async (req, res) => {
    try {
        ...
        const onlineUsers = await models_1.User.find({ isOnline: true })
            .sort({ lastSeen: -1 })
            .limit(Number(limit))
            .skip(Number(offset))
...
router.post('/batch-status', async (req, res) => {
    try {
        ...
        const userStatuses = await models_1.UserStatus.find({
            userId: { $in: userIds }
