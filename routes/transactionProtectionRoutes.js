...
router.get('/check-block-status/:userId/:targetUserId', async (req, res) => {
    try {
        ...
        const [user, targetUser] = await Promise.all([
            models_1.User.findOne({ id: userId }),
            models_1.User.findOne({ id: targetUserId })
        ]);
        if (!user || !targetUser) {
        ...
        const recentWithdrawals = await models_1.PurchaseRecord.find({
            userId: userId,
            type: 'withdrawal',
        ...
        const targetRecentWithdrawals = await models_1.PurchaseRecord.find({
            userId: targetUserId,
            type: 'withdrawal',
            status: { $in: ['Processando', 'pending'] },
        ...
        const recentDisputes = await models_1.PurchaseRecord.find({
            $or: [
                { userId, type: 'dispute' },
        ...
        const recentBlocks = await models_1.PurchaseRecord.find({
            userId,
            type: 'block_attempt',
        ...
        // Registrar tentativa de verificação para auditoria + persistir atividade
        ...
        // Persistir atividade de verificação de bloqueio
...
router.post('/register-block-attempt', async (req, res) => {
    try {
        ...
        // Registrar tentativa de bloqueio + persistir atividade
        await models_1.PurchaseRecord.create({
        ...
        // Persistir atividade de tentativa de bloqueio
        await models_1.User.findOneAndUpdate({ id: userId }, {
            $push: {
                recentActivities: {
