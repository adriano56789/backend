...
router.post('/streams/:id/publish', async (req, res) => {
    try {
        ...
        try {
            const followers = await index_1.Followers.find({
                followingId: hostId,
                isActive: true
            ...
            if (followers.length > 0) {
                ...
                try {
                    ...
                    await NotificationService.notifyLiveStarted(io, hostId, user.name || 'LiveGO', user.avatarUrl || '', id, followerIds);
                }
                catch (notifErr) {
...
router.post('/live/end', async (req, res) => {
    try {
        ...
        await index_1.Streamer.updateOne({ id: activeStream.id }, {
            $set: {
                isLive: false,
                streamStatus: 'ended',
                endTime: new Date()
...
router.post('/lives/:id/end', async (req, res) => {
    try {
        ...
        await index_1.Streamer.findOneAndUpdate({ id: realId }, {
            isLive: false,
            streamStatus: 'ended',
            endTime: new Date()
        });
...
router.post('/stark/live/publish', async (req, res) => {
    try {
        ...
        // Criar/atualizar LiveCard (so quando realmente publicar)
        try {
            const user = await index_1.User.findOne({ id: tokenUserId }).lean();
...
// POST /api/streams/:id/live-message - Enviar mensagem na live
router.post('/streams/:id/live-message', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        try {
            const io = req.app.get('io');
            if (io) io.to(id).emit('live_message', messagePayload);
        } catch (ioErr) {
            console.warn('[LIVE-MESSAGE-REST] Erro ao emitir socket:', ioErr);
        }
        res.json({ success: true, message: 'Mensagem enviada com sucesso', data: messagePayload });
    } catch (error) {
        console.error('[LIVE-MESSAGE-REST] Erro:', error);
        res.status(500).json({ success: false, message: 'Erro interno ao enviar mensagem', error: error instanceof Error ? error.message : String(error) });
    }
});
