...
async function testAPI(api) {
    ...
    try {
        const rawResp = await httpClient_1.httpClient.requestRaw(api.method, `http://localhost:3000${api.url}`, undefined, { headers: {
...
router.get('/', (req, res) => {
    res.sendFile(path_1.default.join(__dirname, '../views/monitor.html'));
});
...
router.post('/scan', async (req, res) => {
    try {
        ...
        console.error('❌ [MONITOR] Erro ao escanear APIs:', error.message);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
router.get('/results', async (req, res) => {
    try {
        ...
        console.error('❌ [MONITOR] Erro ao carregar resultados:', error.message);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
router.post('/test', async (req, res) => {
    try {
        ...
        console.error('❌ [MONITOR] Erro ao testar API:', error.message);
        res.status(500).json({
            success: false,
            error: error.message
        });
