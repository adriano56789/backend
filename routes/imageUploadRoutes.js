...
router.post('/gallery', auth_1.protect, galleryUpload.single('image'), async (req, res) => {
    try {
        ...
        const newPhoto = await models_1.ProfilePhoto.create({
            obraId,
            userId,
            photoUrl: imageUrl,
            photoType: 'gallery',
...
router.post('/video', auth_1.protect, videoUpload.single('video'), async (req, res) => {
    try {
        const userId = (0, auth_1.getUserIdFromToken)(req);
        if (!userId) {
            return res.status(401).json({
        ...
        const newVideo = await models_1.ProfilePhoto.create({
            obraId,
            userId,
            photoUrl: videoUrl,
...
router.get('/user/:userId/images', auth_1.protect, async (req, res) => {
    try {
        ...
        const images = await models_1.ProfilePhoto.find({
            userId,
            isActive: true
...
router.get('/user/:userId/avatar', async (req, res) => {
    try {
        ...
        const avatar = await models_1.ProfilePhoto.findOne({
            userId,
            photoType: 'avatar',
            isMain: true,
            isActive: true
...
router.put('/user/:userId/avatar', auth_1.protect, async (req, res) => {
    try {
        const { userId } = req.params;
        const currentUserId = (0, auth_1.getUserIdFromToken)(req);
        ...
        const updatedUser = await models_1.User.findOneAndUpdate({ id: userId }, { $set: { avatarUrl, updatedAt: new Date() } }, { returnDocument: 'after' });
...
router.delete('/images/:imageId', auth_1.protect, async (req, res) => {
    try {
        ...
        const image = await models_1.ProfilePhoto.findOne({
            id: imageId,
            userId
        });
        if (!image) {
        ...
        const filePath = path_1.default.join(__dirname, '../../uploads', image.photoType, path_1.default.basename(image.photoUrl));
        try {
        ...
        await models_1.ProfilePhoto.deleteOne({ id: imageId });
        res.json({
            success: true,
