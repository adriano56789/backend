...
router.post('/invite', async (req, res) => {
    try {
        ...
        await Promise.all([
            models_1.User.findOneAndUpdate({ id: hostId }, { $push: { recentActivities: { action: 'call_invite_sent', resource: 'video_call', timestamp: new Date(), endpoint: '/api/call-invitation/invite' } } }).catch(() => { }),
            models_1.User.findOneAndUpdate({ id: guestId }, { $push: { recentActivities: { action: 'call_invite_received', resource: 'video_call', timestamp: new Date(), endpoint: '/api/call-invitation/invite' } } }).catch(() => { })
        ]);
...
router.post('/respond', async (req, res) => {
    try {
        ...
        await invitation.save();
        await models_1.User.findOneAndUpdate({ id: userId }, { $push: { recentActivities: { action: response === 'accept' ? 'call_accepted' : 'call_declined', resource: 'video_call', timestamp: new Date(), endpoint: '/api/call-invitation/respond' } } }).catch(() => { });
        const io = req.app.get('io');
...
router.post('/end', async (req, res) => {
    try {
        ...
        await invitation.save();
        await Promise.all([
            models_1.User.findOneAndUpdate({ id: invitation.hostId }, { $push: { recentActivities: { action: 'call_ended', resource: 'video_call', timestamp: new Date(), endpoint: '/api/call-invitation/end' } } }).catch(() => { }),
            models_1.User.findOneAndUpdate({ id: invitation.guestId }, { $push: { recentActivities: { action: 'call_ended', resource: 'video_call', timestamp: new Date(), endpoint: '/api/call-invitation/end' } } }).catch(() => { })
        ]);
        const io = req.app.get('io');
        if (io) {
