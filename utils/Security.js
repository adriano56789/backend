...
class Security {
    ...
    static async blockAbusiveUser(userId, reason, duration = 3600) {
        try {
            ...
            const resp = await httpClient_1.httpClient.requestRaw('POST', '/api/security/block', {
    ...
    static async auditCredentialsUsage(userId, action, metadata = {}) {
        try {
            ...
            const resp = await httpClient_1.httpClient.requestRaw('POST', '/api/security/audit', auditData);
            if (!resp.ok) {
