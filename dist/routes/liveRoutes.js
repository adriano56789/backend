...
router.get('/streams', async (req, res) => {
    try {
        ...
        try {
            ...
            if (srsRes) {
                ...
                for (const srs of srsStreams) {
                    ...
                    if (!streamKey)
                        continue;
                    if (streamKey.endsWith('_transcoded'))
                        continue;
                    const hostId = streamKey.replace('stream_', '');
        ...
        if (hasCountryFilter) {
            const countryFilter = { ...baseFilter, country: country.toLowerCase() };
...
router.get('/lives/:room/livekit-token', async (req, res) => {
    ...
    try {
        ...
        const liveRoomName = await (0, LiveKitTokenService_1.ensureLiveKitRoom)(streamId);
        ...
        const token = await (0, LiveKitTokenService_1.generateLiveKitToken)(identity, liveRoomName, undefined, extraGrants);
        res.json({
            success: true,
            token,
            identity,
