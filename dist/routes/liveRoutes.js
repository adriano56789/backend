...
router.get('/lives/:id/livekit-token', async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            identity,
            room: `stream_${room}`,
            serverUrl: env_1.ENV.LIVEKIT_URL || 'wss://sfu.livego.store',
            livekitUrl: env_1.ENV.LIVEKIT_URL || 'wss://sfu.livego.store',
        });
    }
    catch (error) {
...
router.post('/srs/publish', async (req, res) => {
    try {
        ...
        await index_1.Streamer.findOneAndUpdate({ id: streamId }, streamerData, { upsert: true, new: true });
...
router.get('/streams', async (req, res) => {
    try {
        const { category = 'popular', country = 'all', limit = 50, cursor = '', isLive = 'true', userId } = req.query;
...
router.put('/streams/:id', async (req, res) => {
    try {
        ...
        // Atualizar stream
        const stream = await index_1.Streamer.findOneAndUpdate({ id }, updateData, { new: true, runValidators: true });
        if (!stream) {
...
router.patch('/streams/:id', async (req, res) => {
    ...
    const stream = await index_1.Streamer.findOneAndUpdate({ id: req.params.id }, req.body, { new: true });
    res.json({ success: true, stream });
});
...
router.post('/streams/:id/urls', async (req, res) => {
    try {
        ...
        const updatedStream = await index_1.Streamer.findOneAndUpdate({ id: streamId }, updateData, { new: true });
...
router.post('/streams/:id/cover', async (req, res) => {
    try {
        ...
        const stream = await index_1.Streamer.findOneAndUpdate({ id: req.params.id }, { avatar: coverUrl }, { new: true });
        if (!stream) {
            return res.status(404).json({ success: false, error: 'Stream not found' });
        }
        res.json({ success: true, stream });
...
router.post('/streams/:id/gift', async (req, res) => {
    try {
        ...
        if (fromUserId === stream.hostId) {
            ...
            updatedSender = await U.findOneAndUpdate({ id: fromUserId }, { $inc: { diamonds: -totalValue, enviados: totalValue, receptores: totalValue, earnings: totalValue } }, { new: true });
            ...
            updatedSender = await U.findOneAndUpdate({ id: fromUserId }, { $inc: { diamonds: -totalValue, enviados: totalValue } }, { new: true });
            if (stream.hostId) {
                updatedReceiver = await U.findOneAndUpdate({ id: stream.hostId }, { $inc: { receptores: totalValue, earnings: totalValue } }, { new: true });
...
router.post('/streams/start', async (req, res) => {
    try {
        ...
        const stream = await index_1.Streamer.create({
            id: liveId,
            hostId: userId,
            name: user.name,
            avatar: user.avatarUrl || '',
...
router.post('/lives/start', async (req, res) => {
    try {
        ...
        let stream = await index_1.Streamer.findOne({ streamKey: streamId });
        if (!stream) {
            stream = await index_1.Streamer.findOne({ id: streamId });
        }
        const srsHost = process.env.SRS_HOST || 'srs';
        ...
        if (!stream) {
            ...
            const liveId = (0, uuid_1.v4)();
            stream = await index_1.Streamer.create({
                id: liveId,
                hostId: userId,
                name: user.name,
                avatar: user.avatarUrl || '',
