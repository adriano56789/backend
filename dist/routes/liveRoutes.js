...
router.post('/srs/hook', async (req, res) => {
    try {
        ...
        if (action === 'on_publish') {
            ...
            await index_1.LiveCard.findOneAndUpdate({ hostId }, { $set: { hostId, name: userName, avatar: userAvatar, streamKey, isLive: true, streamStatus: 'active', startTime: new Date(), updatedAt: new Date() } }, { upsert: true }).catch(() => { });
...
router.post('/streams/:id/toggle-mic', async (req, res) => {
    try {
        ...
        await sessions.updateOne({ streamId, endTime: { $exists: false } }, { $set: { isMicrophoneMuted: newMicState, updatedAt: new Date() } });
        const io = req.app.get('io');
        if (io) {
...
router.post('/streams/:id/toggle-sound', async (req, res) => {
    try {
        ...
        await sessions.updateOne({ streamId, endTime: { $exists: false } }, { $set: { isStreamMuted: newSoundState, updatedAt: new Date() } });
        const io = req.app.get('io');
        if (io) {
...
router.get('/lives/:id', async (req, res) => {
    try {
        ...
        const streamer = await index_1.Streamer.findOne({ id });
        if (!streamer) {
            console.log(` Streamer not found: ${id}`);
            return res.status(404).json({ error: 'Streamer not found' });
        }
        // Transformar streamer para formato protegido usando mapper flex+vel
        const protectedStream = (0, srsStreamMapper_1.mapStreamToProtectedFlexible)(streamer);
