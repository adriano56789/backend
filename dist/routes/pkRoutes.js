...
router.get('/config', async (req, res) => {
    ...
    if (userId) {
        await models_1.User.findOneAndUpdate({ id: userId }, { $push: { recentActivities: { action: 'pk_config_viewed', resource: 'pk_battle', timestamp: new Date(), endpoint: '/api/pk/config' } } }).catch(console.error);
    }
...
router.post('/config', async (req, res) => {
    ...
    if (userId) {
        await models_1.User.findOneAndUpdate({ id: userId }, { $push: { recentActivities: { action: 'pk_config_updated', resource: 'pk_battle', timestamp: new Date(), endpoint: '/api/pk/config' } } }).catch(console.error);
    }
...
router.post('/start', async (req, res) => {
    try {
        ...
        await models_1.User.findOneAndUpdate({ id: challengerId }, { $push: { recentActivities: { action: 'pk_battle_started', resource: 'pk_battle', timestamp: new Date(), endpoint: '/api/pk/start' } } }).catch(console.error);
...
router.post('/vote', async (req, res) => {
    try {
        ...
        const updated = await models_1.Battle.findByIdAndUpdate(battleId, { $inc: { [field]: 1 } }, { new: true });
        const io = req.app.get('io');
        if (io && updated) {
...
router.post('/end/:battleId', async (req, res) => {
    try {
        ...
        const updated = await models_1.Battle.findByIdAndUpdate(battleId, update, { new: true })
            .populate('streamerA', 'id name displayName avatarUrl')
            .populate('streamerB', 'id name displayName avatarUrl')
            .populate('winner', 'id name displayName avatarUrl');
        const io = req.app.get('io');
        ...
        const userId = req.body.userId || req.headers['user-id'];
        if (userId) {
            await models_1.User.findOneAndUpdate({ id: userId }, { $push: { recentActivities: { action: 'pk_battle_ended', resource: 'pk_battle', timestamp: new Date(), endpoint: '/api/pk/end' } } }).catch(console.error);
        }
