...
router.put('/:userId/photos/:obraId/set-main', async (req, res) => {
    try {
        ...
        const updatedPhoto = await index_1.ProfilePhoto.findOneAndUpdate({ obraId: obraId }, { isMain: true, updatedAt: new Date() }, { new: true });
        if (updatedPhoto) {
...
router.put('/:userId/photos/:obraId/order', async (req, res) => {
    try {
        ...
        const photo = await index_1.ProfilePhoto.findOneAndUpdate({ obraId: obraId, userId, photoType: 'gallery' }, { $set: { order, updatedAt: new Date() } }, { new: true });
        if (!photo) {
            return res.status(404).json({
                success: false,
...
router.put('/me/photos/:obraId/set-main', auth_1.protect, async (req, res) => {
    try {
        ...
        await index_1.ProfilePhoto.updateMany({ userId, photoType: 'avatar', isMain: true }, { $set: { isMain: false } });
        // Definir esta foto como principal
        const updatedPhoto = await index_1.ProfilePhoto.findOneAndUpdate({ obraId: obraId }, { $set: { isMain: true, updatedAt: new Date() } }, { new: true });
        if (updatedPhoto) {
            // VALIDAÇÃO: Bloquear URLs Base64 - apenas permitir URLs normais
            if (updatedPhoto.photoUrl && updatedPhoto.photoUrl.startsWith('data:image')) {
                return res.status(400).json({
