...
router.post('/', (0, appOwnerProtection_1.blockProtection)(), async (req, res) => {
    try {
        ...
        await Promise.all([
            models_1.User.findOneAndUpdate({ id: blockerId }, {
                $push: {
                    recentActivities: {
                        action: 'block',
            ...
            models_1.User.findOneAndUpdate({ id: blockedId }, {
                $push: {
                    recentActivities: {
                        action: 'blocked_by',
                        resource: 'social_relation',
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await Promise.all([
            models_1.User.findOneAndUpdate({ id: block.blockerId }, {
                $push: {
                    recentActivities: {
                        action: 'unblock',
                        resource: 'social_relation',
            ...
            models_1.User.findOneAndUpdate({ id: block.blockedId }, {
                $push: {
                    recentActivities: {
                        action: 'unblocked_by',
                        resource: 'social_relation',
