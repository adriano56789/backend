...
router.get('/regions', async (req, res) => {
    try {
        ...
        const regionsWithCount = await Promise.all(regions.map(async (region) => {
            const count = await models_1.Streamer.countDocuments({ country: region.code, isLive: true });
...
router.get('/notifications', async (req, res) => {
    try {
        ...
        const notifications = await models_1.LiveNotification.find({ userId }).sort({ createdAt: -1 }).limit(50);
...
router.patch('/notifications/:id/read', async (req, res) => {
    try {
        const { id } = req.params;
        const notification = await models_1.LiveNotification.findOneAndUpdate({ _id: id }, { isRead: true }, { new: true });
        if (!notification) {
...
router.post('/notifications/start-live', async (req, res) => {
    try {
        ...
        if (streamer && streamer.followingList && streamer.followingList.length > 0) {
            const notifications = streamer.followingList.map((followerId) => ({
                userId: followerId,
                type: 'live_started',
