...
router.get('/:userId', async (req, res) => {
    try {
        ...
        // Remover campos que possam conter MongoDB IDs
        const cleanLevelInfo = {
...
router.post('/:userId/add-exp', async (req, res) => {
    try {
        ...
        const result = await userLevel.addExp(amount, reason || 'EXP ganho');
        // Atualizar o nível no usuário principal também + persistir atividade
        await (0, idHelper_1.updateUserByRealId)(models_1.User, userId, {
...
router.post('/:userId/multi-add', async (req, res) => {
    try {
        ...
        for (const gain of expGains) {
            if (gain.amount > 0) {
                const result = await userLevel.addExp(gain.amount, gain.reason || 'EXP ganho');
