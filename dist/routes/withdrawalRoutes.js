...
router.post('/bank', auth_1.protect, rateLimit_1.paymentRateLimit, async (req, res) => {
    try {
        ...
        // Buscar usuário com projeção apenas para dados financeiros
        const user = await models_1.User.findOne({ id: userId }).select('id earnings withdrawal_method country name email');
        if (!user) {
            return res.status(404).json({ error: 'Usuário não encontrado' });
        }
        // Verificar método de saque configurado
        ...
        const currency = (preferredCurrency === 'EUR' || preferredCurrency === 'USD') ? preferredCurrency : (0, currencyService_1.getCurrencyForCountry)(user.country);
        if (currency === 'BRL') {
            return res.status(400).json({
        ...
        const local_net = await (0, currencyService_1.convertBRL)(net_brl, currency);
        const symbol = currencyService_1.CURRENCY_SYMBOLS[currency];
        // Deduzir saldo total do usuário + persistir atividade
        const updatedUser = await models_1.User.findOneAndUpdate({ id: userId }, {
            $inc: { earnings: -amount },
            $set: {
