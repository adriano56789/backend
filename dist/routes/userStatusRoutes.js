...
router.get('/users/:id/status', async (req, res) => {
    try {
        ...
        if (!userStatus) {
            ...
            userStatus = await models_1.UserStatus.findOneAndUpdate({ userId: id }, { $setOnInsert: { userId: id, isOnline: false, lastSeen: new Date() } }, { upsert: true, new: true });
        }
        res.json({
...
router.post('/users/:id/online', async (req, res) => {
    try {
        ...
        await models_1.UserStatus.findOneAndUpdate({ userId: id }, { $set: { isOnline: true, lastSeen: now } }, { upsert: true, new: true });
        await models_1.User.findOneAndUpdate({ id: id }, {
...
router.post('/users/:id/offline', async (req, res) => {
    try {
        ...
        // 1. Atualizar modelo UserStatus
        await models_1.UserStatus.findOneAndUpdate({ userId: id }, { $set: { isOnline: false, lastSeen: now } }, { upsert: true, new: true });
        await models_1.User.findOneAndUpdate({ id: id }, {
...
router.put('/users/:id/status', async (req, res) => {
    try {
        ...
        const userStatus = await models_1.UserStatus.findOneAndUpdate({ userId: id }, { $set: updateData }, { upsert: true, new: true });
        res.json({
            success: true,
