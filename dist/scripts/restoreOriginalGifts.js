...
async function restoreOriginalGifts() {
    try {
        ...
        const gifts = giftsData.map((g) => ({
            ...
            triggersAutoFollow: g.triggersAutoFollow ?? false,
        }));
        console.log(`📦 ${gifts.length} presentes para importar`);
        await models_1.Gift.deleteMany({});
        console.log('🧹 Coleção limpa');
        const result = await models_1.Gift.insertMany(gifts);
        console.log(`✅ ${result.length} presentes inseridos com sucesso`);
    }
    catch (error) {
