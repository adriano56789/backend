...
exports.antiRecordingProtection = antiRecordingProtection;
 ...
 */
function antiRecordingProtection(req, res, next) {
    ...
    const isDirectApiAccess = !referer || (referer.includes('localhost') &&
 ...
 */
function hideSensitiveData(req, res, next) {
...
function removeSensitiveFields(obj) {
    ...
    if (Array.isArray(cleaned.followersList))
        cleaned.followersList = [];
    if (Array.isArray(cleaned.followingList))
        cleaned.followingList = [];
    if (Array.isArray(cleaned.friendsList))
        cleaned.friendsList = [];
    if (Array.isArray(cleaned.blockedUsers))
        cleaned.blockedUsers = [];
    if (Array.isArray(cleaned.messages))
        cleaned.messages = [];
    if (Array.isArray(cleaned.notifications))
        cleaned.notifications = [];
    if (Array.isArray(cleaned.visitors))
        cleaned.visitors = [];
    return cleaned;
}
