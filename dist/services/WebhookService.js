...
class WebhookService {
     ...
     */
    static async handleEvent(body, context) {
        ...
        let streamIndex = streamers.findIndex((s) => s.id === streamId || s.streamKey === streamId || s.hostId === streamId ||
        ...
        const getOrCreateStream = () => {
            ...
            const hostUser = users.find((u) => u.id === finalHostId) || users[0];
        ...
        switch (normalizedEvent) {
            ...
            case 'card removed': {
                if (streamIndex !== -1) {
                    ...
                    saveCollection('streamers', streamers);
                    // Update user state
                    const hostUser = users.find((u) => u.id === hostIdToDelete);
                    if (hostUser) {
                        hostUser.isLive = false;
                        saveCollection('users', users);
                    }
