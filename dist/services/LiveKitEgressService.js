...
class LiveKitEgressService {
    constructor() {
        this.egressClient = new livekit_server_sdk_1.EgressClient(env_1.ENV.LIVEKIT_SERVER_URL, env_1.ENV.LIVEKIT_API_KEY, env_1.ENV.LIVEKIT_API_SECRET);
    }
    async startRTMPEgress(roomId, streamId, rtmpUrl) {
        try {
            console.log(`[EGRESS] Iniciando RTMP Egress para sala ${roomId} -> SRS ${streamId}`);
            const streamOutput = new protocol_1.StreamOutput({
                protocol: protocol_1.StreamProtocol.RTMP,
                urls: [rtmpUrl],
            });
            const egressInfo = await this.egressClient.startRoomCompositeEgress(roomId, streamOutput, {
                layout: 'speaker',
    ...
    async getEgressStatus(egressId) {
        try {
            ...
            const egressInfo = allEgresses.find((e) => e.egressId === egressId);
            if (!egressInfo) {
                return {
                    success: false,
...
exports.egressService = new LiveKitEgressService();
