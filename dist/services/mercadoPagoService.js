...
function calcNetAmount(payment) {
    const fees = (payment.fee_details || []).reduce((sum, fee) => sum + (fee.amount || 0), 0);
    return (payment.transaction_amount || 0) - fees;
}
function toWithdrawalResponse(payment) {
    return {
...
class MercadoPagoService {
    ...
    async makeWithdrawal(request) {
        try {
            ...
            const payment = await mercadoPago_1.paymentClient.create({
                body: {
    ...
    async getPaymentStatus(paymentId) {
        try {
            const payment = await mercadoPago_1.paymentClient.get({ id: paymentId });
            return toWithdrawalResponse(payment);
        }
        catch (error) {
    ...
    async cancelPayment(paymentId) {
        try {
            await mercadoPago_1.paymentClient.cancel({ id: paymentId });
