...
class FriendshipService {
    ...
    static async createFriendshipRequest(data) {
        try {
            ...
            const existingRequest = await Friendship_1.Friendship.findOne({
                $or: [
                    { userId1: fromUserId, userId2: toUserId },
                    { userId1: toUserId, userId2: fromUserId }
                ],
            ...
            const followFromTo = await Followers_1.Followers.findOne({
                followerId: fromUserId,
                followingId: toUserId,
                isActive: true
            });
            const followToFrom = await Followers_1.Followers.findOne({
                followerId: toUserId,
     ...
     */
    static async acceptFriendshipRequest(friendshipId, userId) {
        try {
            ...
            await User_1.User.findOneAndUpdate({ id: friendship.userId1 }, { $push: { friendsList: friendship.userId2 } });
            await User_1.User.findOneAndUpdate({ id: friendship.userId2 }, { $push: { friendsList: friendship.userId1 } });
     ...
     */
    static async rejectFriendshipRequest(friendshipId, userId) {
        try {
            const friendship = await Friendship_1.Friendship.findOneAndDelete({
                _id: friendshipId,
                $or: [{ userId1: userId }, { userId2: userId }],
                isActive: false
            });
    ...
    static async removeFriendship(data) {
        try {
            ...
            await User_1.User.findOneAndUpdate({ id: userId1 }, { $pull: { friendsList: userId2 } });
            await User_1.User.findOneAndUpdate({ id: userId2 }, { $pull: { friendsList: userId1 } });
    ...
    static async getFriends(userId, limit = 50, offset = 0) {
        try {
            const friendships = await Friendship_1.Friendship.find({
                $or: [
     ...
     */
    static async getPendingFriendshipRequests(userId) {
        try {
            const requests = await Friendship_1.Friendship.find({
                $or: [
    ...
    static async createAutomaticFriendship(data) {
        try {
            ...
            if (!existingFriendship) {
                ...
                // Adicionar às listas de amigos
                await User_1.User.findOneAndUpdate({ id: userId1 }, { $push: { friendsList: userId2 } });
                await User_1.User.findOneAndUpdate({ id: userId2 }, { $push: { friendsList: userId1 } });
