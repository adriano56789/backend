...
const initSocket = (server) => {
    ...
    io.on('connection', (socket) => {
        ...
        if (userId) {
            const now = new Date();
            models_1.UserStatus.findOneAndUpdate({ userId }, { $set: { isOnline: true, lastSeen: now } }, { upsert: true }).then(() => {
            ...
            models_1.User.findOneAndUpdate({ id: userId }, { $set: { isOnline: true, lastSeen: now } }, { upsert: true }).then(() => {
        ...
        socket.on('disconnect', (reason) => {
            ...
            if (userId) {
                const now = new Date();
                models_1.User.findOne({ id: userId }).select('name').lean().then(user => {
                    const userName = user?.name || userId;
                    Promise.resolve().then(() => __importStar(require('./services/PresenceService'))).then(({ PresenceService }) => {
        ...
        socket.on('user_app_open', async () => {
            if (userId) {
                ...
                const userName = user?.name || userId;
                const { PresenceService } = await Promise.resolve().then(() => __importStar(require('./services/PresenceService')));
        ...
        socket.on('send_chat_message', async (data) => {
            ...
            try {
                (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackBeforeSendMessage', { RoomId: data.streamId, FromUserId: data.userId, FromUserName: data.userName || 'Usuário', Content: data.message || '', MsgType: 'normal', Timestamp: Date.now() });
            }
            catch (e) {
                console.warn('[WEBHOOK] before chat message', e);
            }
            io.to(data.streamId).emit('live_message', messagePayload);
            // 🪝 Webhook LiveGo: mensagem enviada (caminho legado send_chat_message)
            try {
                (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterSendMessage', { RoomId: data.streamId, FromUserId: data.userId, FromUserName: data.userName || 'Usuário', Content: data.message || '', MsgType: 'normal', Timestamp: Date.now() });
            }
            catch (e) {
                console.warn('[WEBHOOK] chat message', e);
            }
        ...
        socket.on('send_gift', async (data) => {
            ...
            try {
                ...
                if (data.fromUserId && totalValue > 0) {
                    ...
                    const senderUpdate = await User.findOneAndUpdate({ id: data.fromUserId }, { $inc: { diamonds: -totalValue, enviados: totalValue } }, { returnDocument: 'after' });
                    if (!senderUpdate) {
                    ...
                    if (data.streamId) {
                        ...
                        if (stream && stream.hostId && stream.hostId !== data.fromUserId) {
                            ...
                            const receiverUpdate = await User.findOneAndUpdate({ id: stream.hostId }, { $inc: { receptores: totalValue, diamonds: totalValue, earnings: totalValue } }, { returnDocument: 'after' });
                            if (!receiverUpdate) {
                ...
                try {
                    ...
                    if (stream) {
                        io.to(data.streamId).emit('live_coins_updated', {
                            streamId: data.streamId,
                            coins: stream.diamonds || 0,
                            totalCoins: stream.diamonds || 0,
                            timestamp: new Date().toISOString(),
                        });
                    }
                }
        ...
        socket.on('user_joined', async (data) => {
            ...
            if (data.streamId) {
                try {
                    ...
                    try {
                        const StreamerModel = (await Promise.resolve().then(() => __importStar(require('./models/Streamer')))).Streamer;
                        const streamDoc = await StreamerModel.findOne({ id: data.streamId }).select('hostId').lean();
                        if (streamDoc && streamDoc.hostId)
                            rouletteOwnerId = streamDoc.hostId;
                    }
                    catch { }
                    const items = await (0, RouletteItem_1.findActiveByOwner)(rouletteOwnerId);
                    const rouletteUserDoc = await models_1.User.findOne({ id: rouletteOwnerId }).exec();
        ...
        socket.on('request_roulette_state', async (data) => {
            ...
            try {
                ...
                const rouletteUserDoc = await models_1.User.findOne({ id: ownerId }).exec();
                const spinCost = rouletteUserDoc && Number(rouletteUserDoc.rouletteSpinCost) > 0 ? Number(rouletteUserDoc.rouletteSpinCost) : 0;
                socket.emit('roulette_updated', {
                    ownerId,
                    items: items.map((it) => JSON.parse(JSON.stringify(it && it.toObject ? it.toObject() : it))),
        ...
        socket.on('friendship_request', async (data) => {
            try {
                ...
                try {
                    ...
                    if (fromUser) {
                        await NotificationService.notifyFriendInvite(io, data.toUserId, data.fromUserId, fromUser.name || 'Alguém', fromUser.avatarUrl || '', request._id || request.id || '', data.message);
                    }
                }
                catch (notifErr) {
        ...
        socket.on('live_broadcast', async (data) => {
            try {
                const broadcastData = {
                    type: data.type,
                    streamId: data.streamId,
