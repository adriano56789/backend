...
exports.UserRoutes.get('/available', async (req, res) => {
    try {
        ...
        if (userId) {
            const currentUser = await models_1.User.findOne({ id: userId }).select('blockedUsers').lean();
            const blockedByMe = currentUser?.blockedUsers || [];
            // Também buscar quem bloqueou o usuário
            const blockers = await models_1.User.find({
                blockedUsers: userId
            }).select('id').lean();
            const blockedMe = blockers.map((b) => b.id);
            const excludeIds = [userId, ...blockedByMe, ...blockedMe];
    ...
    try {
        ...
        if (paramId.length === 24 && /^[0-9a-fA-F]{24}$/.test(paramId)) {
            ...
            if (user) {
                ...
                try {
                    ...
                    if ((userObj.following ?? 0) !== realFollowingEarly)
                        userObj.following = realFollowingEarly;
                    if ((userObj.fans ?? 0) !== realFansEarly) {
                        ...
                        models_1.User.updateOne({ id: user.id }, { $set: { fans: userObj.fans } }).catch(() => { });
                    }
                }
        ...
        if (user) {
            ...
            if ((userObj.fans ?? 0) !== computedFans) {
                ...
                models_1.User.updateOne({ id: user.id }, { $set: { fans: computedFans } }).catch(() => { });
            }
...
exports.UserRoutes.patch("/:id", async (req, res) => {
    try {
        ...
        let user = await models_1.User.findOneAndUpdate({ id: paramId }, updateData, { returnDocument: 'after' });
        if (!user) {
            user = await models_1.User.findOneAndUpdate({ id: { $regex: new RegExp('^' + paramId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '$', 'i') } }, updateData, { returnDocument: 'after' });
        }
        if (!user) {
...
exports.UserRoutes.delete('/:userId/photos/:photoId', async (req, res) => {
    try {
        ...
        const updated = await models_1.User.findOneAndUpdate({ id: userId }, { $set: { obras: newObras, avatarUrl: newAvatarUrl } }, { returnDocument: 'after' });
...
exports.UserRoutes.post('/:id/toggle-follow', async (req, res) => {
    try {
        ...
        if (existingFollow) {
            ...
            try {
                (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterUnfollow', { FollowerId: followerId, FollowedId: followingId, Timestamp: Date.now() });
            }
            catch (e) {
                console.warn('[WEBHOOK] unfollow', e);
            }
            res.json({
                success: true,
                isFollowing: false,
            ...
            try {
                (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterFollow', { FollowerId: followerId, FollowedId: followingId, Timestamp: Date.now() });
            }
            catch (e) {
                console.warn('[WEBHOOK] follow', e);
            }
            res.json({
                success: true,
                isFollowing: true,
...
exports.UserRoutes.delete('/:id/messages/:partnerId', async (req, res) => {
    try {
        const userId = req.params.id;
        const partnerId = req.params.partnerId;
        // Segurança: apenas o próprio dono do histórico pode apagá-lo
        const requesterId = (0, auth_1.getUserIdFromToken)(req) || req.query.requesterId;
        ...
        try {
            await Chat.updateMany({ participants: { $all: [userId, partnerId] } }, { $set: { lastMessage: null, updatedAt: new Date() } });
        }
        catch (chatErr) {
...
exports.UserRoutes.post('/:id/buy-diamonds', async (req, res) => {
    try {
        ...
        const user = await models_1.User.findOneAndUpdate({ id: req.params.id }, { $inc: { diamonds: amount } }, { returnDocument: 'after' });
...
exports.UserRoutes.post('/:id/location-permission', async (req, res) => {
    const user = await models_1.User.findOneAndUpdate({ id: req.params.id }, { $set: { locationPermission: req.body.status } }, { returnDocument: 'after' });
    res.json({ success: !!user, user: (0, userResponse_1.standardizeUserResponse)(user) || {} });
});
exports.UserRoutes.post('/:id/privacy/activity', async (req, res) => {
    const user = await models_1.User.findOneAndUpdate({ id: req.params.id }, { $set: { showActivityStatus: req.body.show } }, { returnDocument: 'after' });
    res.json({ success: !!user, user: (0, userResponse_1.standardizeUserResponse)(user) || {} });
});
exports.UserRoutes.post('/:id/privacy/location', async (req, res) => {
    const user = await models_1.User.findOneAndUpdate({ id: req.params.id }, { $set: { showLocation: req.body.show } }, { returnDocument: 'after' });
    res.json({ success: !!user, user: (0, userResponse_1.standardizeUserResponse)(user) || {} });
});
...
exports.UserRoutes.post('/:id/set-active-frame', async (req, res) => {
    const user = await models_1.User.findOneAndUpdate({ id: req.params.id }, { $set: { activeFrameId: req.body.frameId } }, { returnDocument: 'after' });
    res.json({ success: !!user, user: (0, userResponse_1.standardizeUserResponse)(user) });
});
...
exports.UserRoutes.post('/:userId/frames/equip', async (req, res) => {
    try {
        ...
        const updatedUser = await models_1.User.findOneAndUpdate({ id: userId }, { $set: { activeFrameId: frameId } }, { returnDocument: 'after' });
...
exports.UserRoutes.post('/:userId/frames/unequip', async (req, res) => {
    try {
        ...
        const updatedUser = await models_1.User.findOneAndUpdate({ id: userId }, { $set: { activeFrameId: null } }, { returnDocument: 'after' });
