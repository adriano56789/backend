...
router.post('/stream/webrtc/publish', async (req, res) => {
    try {
        ...
        if (result.code === 0) {
            ...
            // Atualizar status do stream
            await Streamer_1.Streamer.findOneAndUpdate({ id: streamId }, { $set: { isLive: true, streamStatus: 'active', startTime: new Date(), kickedUsers: [], webrtcSessionId: result.sessionid } });
...
router.post('/rtc/v1/publish', async (req, res) => {
    try {
        ...
        if (result.code === 0) {
            ...
            if (streamKey) {
                await Streamer_1.Streamer.findOneAndUpdate({ id: streamKey }, { $set: { isLive: true, streamStatus: 'active', startTime: new Date(), kickedUsers: [], webrtcSessionId: result.sessionid } });
                // Criar/atualizar LiveCard
                try {
...
router.get('/rtc/ice-servers', (req, res) => {
    ...
    const backendUrl = (env_1.ENV.BACKEND_URL || process.env.BACKEND_URL || '').replace(/\/+$/, '');
