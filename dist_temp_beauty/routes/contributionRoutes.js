...
router.get('/contribution/:period', async (req, res) => {
    try {
        ...
        const { userId } = req.query; // ID do usuário que está acessando o ranking
        ...
        if (period === 'live' || period === 'Ao vivo') {
            ...
            const recentGifts = await models_1.GiftTransaction.find({
            ...
            const streamers = await models_1.User.find({ id: { $in: streamerIds } });
