...
router.get('/notifications/public-key', (_req, res) => {
    const publicKey = (0, webPushService_1.getPublicKey)();
    if (!publicKey) {
...
router.post('/notifications/register-token', async (req, res) => {
    try {
        ...
        if (subscription?.endpoint && subscription?.keys?.p256dh && subscription?.keys?.auth) {
            ...
            await DeviceToken_1.DeviceToken.findOneAndUpdate({ endpoint }, { $set: { userId, token: tokenJson, endpoint, platform } }, { upsert: true, returnDocument: 'after' });
...
router.delete('/notifications/unregister-token', async (req, res) => {
    try {
        ...
        if (endpoint) {
            await DeviceToken_1.DeviceToken.deleteOne({ endpoint });
        }
        else {
...
router.post('/notifications/send', async (req, res) => {
    try {
        ...
        const failed = await (0, webPushService_1.sendPushNotificationToMultiple)(tokenList, { title, body, data });
        if (failed.length > 0) {
