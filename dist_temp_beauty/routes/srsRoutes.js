...
router.post('/publish', async (req, res) => {
    try {
        ...
        if ((0, streamKeyUtils_1.isTranscodeVariant)(realStreamKey)) {
        ...
        try {
            ...
            // 🔒 Sala privada: espelhar isPrivate + convidados do Streamer (fonte da
            // verdade) no LiveCard — a listagem da categoria private lê o LiveCard.
            const streamMeta = await models_1.Streamer.findOne({ hostId: userId })
                .select('isPrivate invitedUsers').lean().catch(() => null);
            await LiveCard.findOneAndUpdate({ hostId: userId }, { $set: {
                    ...
                    startTime: existingStream?.startTime || new Date(),
                    updatedAt: new Date(),
                    ...(streamMeta && typeof streamMeta.isPrivate === 'boolean' ? { isPrivate: streamMeta.isPrivate } : {}),
                    ...(streamMeta && Array.isArray(streamMeta.invitedUsers) ? { invitedUsers: streamMeta.invitedUsers } : {}),
                    ...(user && typeof user.screenSecurityEnabled === 'boolean' ? { screenSecurityEnabled: user.screenSecurityEnabled } : {})
                } }, { upsert: true });
        }
        catch (cardErr) {
...
router.post('/unpublish', async (req, res) => {
    try {
        const { client_id, stream } = req.body;
        const realStreamKey = stream?.split('?')[0] || stream;
        ...
        if (realStreamKey) {
            ...
            const timer = setTimeout(() => {
                ...
                (0, streamEndService_1.autoEndStreamOnDisconnect)(realStreamKey, io);
            }, RECONNECT_WINDOW_MS);
...
router.post('/play', async (req, res) => {
    try {
        ...
        if ((0, streamKeyUtils_1.isTranscodeVariant)(playStreamKey)) {
            return res.status(200).json({ code: 0 });
        }
...
router.post('/stop', async (req, res) => {
    try {
        ...
        if ((0, streamKeyUtils_1.isTranscodeVariant)(stopStreamKey)) {
            return res.status(200).json({ code: 0 });
        }
...
router.post('/hls', async (req, res) => {
    try {
        ...
        if ((0, streamKeyUtils_1.isTranscodeVariant)(hlsStreamKey)) {
            return res.status(200).json({ code: 0 });
        }
