...
router.delete('/imagens/:id', async (req, res) => {
    try {
        ...
        const photo = await models_1.ProfilePhoto.findOneAndUpdate({ obraId: req.params.id, userId }, { $set: { isActive: false } }, { returnDocument: 'after' });
        if (!photo) {
...
router.put('/imagens/ordenar', async (req, res) => {
    try {
        ...
        const updatePromises = orderedIds.map((photoId, index) => models_1.Photo.findOneAndUpdate({ id: photoId, userId }, { $set: { order: index } }, { returnDocument: 'after' }));
...
singleValueRoutes.forEach(({ route, field }) => {
    ...
    router.put(`/${route}`, async (req, res) => {
        try {
            ...
            res.json({ success: true, user: (0, userResponse_1.standardizeUserResponse)(user), field, value: user[field] });
        }
        catch (error) {
            res.status(500).json({ error: error.message });
        }
...
router.put('/residencia/:userId', async (req, res) => {
    try {
        ...
        if (residenceValue !== undefined)
            updateFields.residence = residenceValue;
        if (city !== undefined)
            updateFields.city = city;
        if (state !== undefined)
            updateFields.state = state;
        const user = await models_1.User.findOneAndUpdate({ id: userId }, { $set: updateFields }, { returnDocument: 'after' });
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        const io = req.app.get('io');
