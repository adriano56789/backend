...
const srsHookReconnectTimers = new Map();
...
async function clearLiveChat(streamId) {
    if (!streamId)
        return;
    try {
        const result = await index_1.LiveMessage.deleteMany({ streamId: String(streamId) });
...
const isValidObjectId = (value) => { try {
...
router.post('/token/user/online/infos', async (req, res) => {
    try {
        ...
        const hasActiveStream = await index_1.Streamer.findOne({
            hostId: userId,
            isLive: true,
            streamStatus: 'active'
        });
...
router.get('/live/info', async (req, res) => {
    try {
        ...
        const { liveId, streamId, likeNum = '0', pushIp, indexs = '1' } = req.query;
        console.log('[LIVEGO-INFO] Parâmetros:', { liveId, streamId, likeNum, pushIp, indexs });
        // Buscar stream - priorizar streamId ou liveId
        let stream;
        if (streamId) {
            stream = await index_1.Streamer.findOne({ id: streamId });
        }
        else if (liveId) {
            stream = await index_1.Streamer.findOne({ liveId: liveId });
        }
        else {
            // Se não especificar, buscar live atual do usuário
            stream = await index_1.Streamer.findOne({
                hostId: userId,
                isLive: true
            });
        }
        ...
        const host = await (0, idHelper_1.findUserByAnyId)(index_1.User, stream.hostId);
        if (!host) {
            return res.status(404).json({
                code: 1,
...
router.post('/live/beat', async (req, res) => {
    try {
        ...
        // Atualizar heartbeat
        await index_1.Streamer.findOneAndUpdate({ id: stream.id }, {
        ...
        await index_1.User.findOneAndUpdate({ id: userId }, { $set: { lastSeen: new Date() } });
...
router.post('/live/monitoring/success', async (req, res) => {
    try {
        ...
        const { liveId, streamId, startTime, quality, bitrate, resolution, fps, droppedFrames } = req.body;
        // Buscar stream
        let stream;
        if (streamId) {
            stream = await index_1.Streamer.findOne({ id: streamId });
        ...
        await index_1.Streamer.findOneAndUpdate({ _id: stream._id }, { $set: {
                streamingSuccess: true,
...
function buildCountryFilter(countryValue) {
    ...
    for (const [key, code] of Object.entries(COUNTRY_NAME_TO_CODE)) {
        if (code === target && !variants.includes(key))
            variants.push(key);
    }
    return { $in: variants };
}
...
router.post('/srs/hook', async (req, res) => {
    try {
        ...
        if (action === 'on_publish') {
            ...
            try {
                const u = await index_1.User.findOne({ id: hostId }).lean();
                if (u) {
                    userName = u.name || hostId;
                    userAvatar = u.avatarUrl || '';
                }
            }
            catch (_) { }
            await index_1.Streamer.findOneAndUpdate({ $or: [{ id: hostId }, { streamKey }] }, { $set: { isLive: true, streamStatus: 'active', streamKey, id: hostId } }, { upsert: true }).catch(() => { });
            await index_1.User.findOneAndUpdate({ id: hostId }, { $set: { isLive: true, currentStreamId: hostId } }).catch(() => { });
            await index_1.LiveCard.findOneAndUpdate({ hostId }, { $set: { hostId, name: userName, avatar: userAvatar, streamKey, isLive: true, streamStatus: 'active', startTime: new Date(), kickedUsers: [], updatedAt: new Date() } }, { upsert: true }).catch(() => { });
...
router.post('/srs/publish', async (req, res) => {
    try {
        ...
        await index_1.Streamer.findOneAndUpdate({ id: streamId }, streamerData, { upsert: true, returnDocument: 'after' });
...
router.post('/live/clear', async (req, res) => {
    try {
        ...
        for (const stream of activeStreams) {
            await index_1.Streamer.updateOne({ _id: stream._id }, { $set: {
                    isLive: false,
                    streamStatus: 'ended',
                    endTime: new Date()
                } });
...
router.post('/live/start', async (req, res) => {
    ...
    try {
        ...
        const newStream = await index_1.Streamer.findOneAndUpdate({ hostId: userId }, { $set: streamerData }, { upsert: true, returnDocument: 'after', setDefaultsOnInsert: true });
        ...
        await UserStatus.findOneAndUpdate({ userId: userId }, { $set: {
                isOnline: true,
                lastSeen: new Date(),
                updatedAt: new Date()
...
router.post('/streams/:id/join', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        // Buscar stream
        const stream = await index_1.Streamer.findOne({ id });
        if (!stream) {
            return res.status(404).json({
                success: false,
        ...
        const viewer = await index_1.User.findOne({ id: userId });
        if (!viewer) {
            return res.status(404).json({
                success: false,
                message: 'Usuário não encontrado'
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterMemberStatusChange', { RoomId: id, UserId: userId, Status: 'online', Action: 'join', Timestamp: Date.now() });
        }
        catch (e) {
        ...
        try {
            const sAfter = await index_1.Streamer.findOne({ id }).select('viewers maxViewers').lean();
            const maxV = (sAfter && sAfter.maxViewers) || 0;
            const v = (sAfter && sAfter.viewers) || 0;
            if (maxV > 0 && v / maxV >= 0.7) {
                (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterCreateRoomReachingThreshold', { RoomId: id, Viewers: v, MaxViewers: maxV, Percent: Math.round((v / maxV) * 100), Timestamp: Date.now() });
            }
        }
        catch (e) {
        ...
        try {
            ...
            await LiveUser.findOneAndUpdate({ userId }, {
                userId,
                username: userId,
...
router.post('/streams/:id/publish-token', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        // Buscar stream
        const stream = await index_1.Streamer.findOne({ id, hostId: userId });
        if (!stream) {
            return res.status(404).json({
                success: false,
                message: 'Stream não encontrado'
        ...
        const srsApiUrl = env_1.ENV.SRS_API_URL;
...
router.post('/streams/:id/end', async (req, res) => {
    try {
        ...
        let stream = await index_1.Streamer.findOne({ id, hostId: userId });
        if (!stream) {
            stream = await index_1.Streamer.findOne({ id });
            if (stream) {
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterDestroyRoom', { RoomId: stream.id || id, HostId: stream.hostId || userId, EndedBy: 'owner', EndTime: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] room destroyed (end)', e);
        }
        const io = req.app.get('io');
        if (io) {
            io.emit('card_removed', {
...
router.post('/streams', async (req, res) => {
    try {
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterCreateRoom', { RoomId: streamId, RoomName: streamTitle, HostId: hostId, Owner_Account: hostId, RoomType: 'Live', StreamKey: streamKey, Category: category, Status: 'preparing', CreateTime: Math.floor(Date.now() / 1000), EventTime: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] room created', e);
        }
...
router.get('/streams', async (req, res) => {
    try {
        const { category = 'popular', country = 'all', limit = 50, cursor = '', isLive = 'true', userId, latitude, longitude } = req.query;
        ...
        try {
            ...
            const { httpClient } = await Promise.resolve().then(() => __importStar(require('../utils/httpClient')));
            const srsRes = await httpClient.get(srsUrl, { timeout: 5000 });
            if (srsRes) {
                const srsData = srsRes;
                const srsStreams = srsData?.streams || [];
                for (const srs of srsStreams) {
                    if (!srs.publish?.active)
                        continue;
                    ...
                    const exists = await index_1.LiveCard.findOne({ hostId, isLive: true, streamStatus: { $in: ['active', 'live'] } }).lean();
                    if (!exists) {
                        const user = await index_1.User.findOne({ id: hostId }).lean();
                        await index_1.LiveCard.findOneAndUpdate({ hostId }, { $set: {
                                hostId,
                                ...
                                isLive: true,
                                streamStatus: 'active',
                                category: user?.category || 'popular',
                                startTime: new Date(),
                                updatedAt: new Date()
                            } }, { upsert: true });
                        await index_1.Streamer.findOneAndUpdate({ id: hostId }, { $set: { isLive: true, streamStatus: 'active', streamKey } }, { upsert: true });
                        const { StreamRoom } = await Promise.resolve().then(() => __importStar(require('../models/index')));
                        await StreamRoom.findOneAndUpdate({ roomId }, { $set: { roomId, hostId, streamKey, app } }, { upsert: true });
        ...
        const baseFilter = {
            streamKey: { $not: streamKeyUtils_1.TRANSCODE_VARIANT_REGEX }
        };
        if (isLive === 'true') {
        ...
        let sortField = { viewers: -1, startTime: -1, _id: -1 };
        ...
        if (userId) {
            try {
                const { StreamBan } = await Promise.resolve().then(() => __importStar(require('../models')));
                ...
                if (bannedHostIds.length > 0) {
                    if (Array.isArray(baseFilter.hostId?.$in)) {
                        baseFilter.hostId.$in = baseFilter.hostId.$in.filter((id) => !bannedHostIds.includes(String(id)));
                    }
                    else {
        ...
        if (cursor) {
            baseFilter._id = { $lt: new mongodb_1.ObjectId(cursor) };
        }
        let cardDocs = [];
        if (useGeoSearch && latitude && longitude) {
            ...
            if (!isNaN(lat) && !isNaN(lng)) {
                const nearbyUsers = await index_1.User.find({
        ...
        if (hasCountryFilter) {
            ...
            cardDocs = await index_1.LiveCard.find(countryFilter)
                .sort(sortField)
                .limit(parseLimit + 1)
                .lean();
            ...
            cardDocs = await index_1.LiveCard.find(baseFilter)
                .sort(sortField)
                .limit(parseLimit + 1)
                .lean();
        }
        if (!hasCountryFilter && cardDocs.length === 0 && isLive === 'true') {
            ...
            cardDocs = await index_1.LiveCard.find(fallbackFilter)
                .sort(sortField)
                .limit(parseLimit + 1)
                .lean();
        }
        if (!hasCountryFilter && cardDocs.length === 0 && isLive === 'true') {
            cardDocs = await index_1.LiveCard.find({
                isLive: true,
...
router.get('/v1/streams', async (req, res) => {
    try {
        ...
        const streams = await index_1.Streamer.find({
            isLive: true,
            streamStatus: 'active'
        })
            .sort({ viewers: -1 })
        ...
        const protectedStreams = (0, srsStreamMapper_1.mapStreamsToProtectedArrayFlexible)(streams);
        res.json({
            success: true,
...
router.post('/end-session', async (req, res) => {
    try {
        ...
        const result = await index_1.Streamer.updateMany({ hostId: userId, isLive: true }, { $set: {
                isLive: false,
                streamStatus: 'ended',
                endTime: new Date()
            } });
...
router.delete('/rtc/v1/stop', async (req, res) => {
    try {
        ...
        let stream = await index_1.Streamer.findOne({ id: streamId, hostId: userId });
        if (!stream) {
...
router.post('/stream-association', async (req, res) => {
    try {
        ...
        // Buscar usuário
        const user = await index_1.User.findOne({ id: userId });
        if (!user) {
            return res.status(404).json({
                success: false,
                message: 'Usuário não encontrado'
        ...
        const existingAssociation = await index_1.StreamKeyAssociation.findOne({ streamKey: sanitizedStreamKey });
        ...
        // Criar associação
        const association = await index_1.StreamKeyAssociation.create({
...
router.post('/start', async (req, res) => {
    try {
        ...
        if (!userId) {
            return responseHelper_1.ResponseHelper.error(res, 'Unauthorized - Token inválido', 401);
        }
        if (!name) {
        ...
        const SRS_API_URL = env_1.ENV.SRS_API_URL;
        const backendHttp = `${BACKEND_URL}/api/video/http`;
        // Criar/Atualizar stream no banco (sem restrição de live ativa)
        const stream = await index_1.Streamer.findOneAndUpdate({ hostId: userId }, { $set: {
                id: streamId,
                hostId: userId,
                name,
                tags,
        ...
        responseHelper_1.ResponseHelper.success(res, {
            stream: {
                id: stream.id,
                streamId: streamId,
        ...
        responseHelper_1.ResponseHelper.error(res, error.message);
    }
});
/**
...
router.get('/:streamId', async (req, res) => {
    try {
        ...
        const stream = await index_1.Streamer.findOne({
            $or: [
                { id: streamId },
                { roomId: streamId }
            ]
        ...
        if (!stream) {
            return responseHelper_1.ResponseHelper.error(res, 'Live não encontrada', 404);
        }
        responseHelper_1.ResponseHelper.success(res, {
            stream: {
                id: stream.id,
...
router.post('/live/:streamId/end', async (req, res) => {
    try {
        ...
        const stream = await index_1.Streamer.findOneAndUpdate({
            $or: [
...
router.get('/stream-info', async (req, res) => {
    try {
        ...
        const association = await index_1.StreamKeyAssociation.findOne({ streamKey, isActive: true });
        if (!association) {
            return res.status(404).json({
                success: false,
        ...
        const user = await index_1.User.findOne({ id: association.userId });
        if (!user) {
            return res.status(404).json({
                success: false,
...
router.post('/live/end', async (req, res) => {
    try {
        ...
        const activeStream = await index_1.Streamer.findOne({
            hostId: userId,
            isLive: true
        });
        if (!activeStream) {
        ...
        try {
            await index_1.Streamer.updateOne({ id: streamId }, {
                $set: {
                    isLive: false,
                    streamStatus: 'ended',
                    endTime: new Date()
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterDestroyRoom', { RoomId: streamId, HostId: userId, EndedBy: 'owner', EndTime: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] room destroyed (live/end)', e);
        }
        ...
        try {
            const activeBattle = await index_1.Battle.findOne({
                $or: [
                    { streamerA: userId },
                    { streamerB: userId },
                    { streamerA: streamId },
            ...
            if (activeBattle) {
                const now = new Date();
                await index_1.Battle.findOneAndUpdate({ _id: activeBattle._id }, {
                    status: 'finished',
                    endedAt: now,
                    winner: null
                });
        ...
        try {
            await index_1.LiveCard.findOneAndUpdate({ hostId: userId }, { $set: {
                    isLive: false,
                    streamStatus: 'ended',
                    endTime: new Date(),
                    updatedAt: new Date()
...
router.get('/live/:category', async (req, res) => {
    try {
        ...
        const isRecordingAttempt = recordingIndicators.some(indicator => userAgent.toLowerCase().includes(indicator.toLowerCase()));
        ...
        if (category === 'global' || category === 'popular') {
            ...
            const streams = await index_1.Streamer.find(filter).sort({ viewers: -1 });
            ...
            const srsStreamsData = (0, srsStreamMapper_1.mapSrsStreamsArray)(streams);
        ...
        const categoryStreams = await index_1.Streamer.find(categoryFilter).sort({ viewers: -1 });
        ...
        const protectedCategoryStreams = (0, srsStreamMapper_1.mapStreamsToProtectedArrayFlexible)(categoryStreams);
...
router.get('/streams/live', async (req, res) => {
    try {
        ...
        const streamsWithHostData = (0, srsStreamMapper_1.enrichStreamsWithHostData)(activeStreams, {
            name: 'Usu+�rio',
            avatar: '',
            level: 1,
            country: 'XX',
...
router.put('/streams/:id', async (req, res) => {
    try {
        ...
        if (updateData.country)
            updateData.country = updateData.country.toLowerCase();
        // Verificar se stream existe
        const existingStream = await index_1.Streamer.findOne({ id });
        if (!existingStream) {
            return res.status(404).json({ error: 'Stream not found' });
        }
        // Atualizar stream
        const stream = await index_1.Streamer.findOneAndUpdate({ id }, updateData, { returnDocument: 'after', runValidators: true });
        if (!stream) {
        ...
        const frontendStream = (0, srsStreamMapper_1.mapSrsStreamToFrontend)(stream);
...
router.patch('/streams/:id', async (req, res) => {
    ...
    const stream = await index_1.Streamer.findOneAndUpdate({ id: req.params.id }, req.body, { returnDocument: 'after' });
    res.json({ success: true, stream });
});
// GET /api/streams/:id/urls - Obter configura+�+�es de URLs de um stream
// ===== ROUTE START =====
router.get('/streams/:id/urls', async (req, res) => {
    try {
        const userId = (0, auth_1.getUserIdFromToken)(req);
        const { id: streamId } = req.params;
        // Verificar se a stream pertence ao usu+�rio
        const stream = await index_1.Streamer.findOne({ id: streamId, hostId: userId });
        if (!stream) {
            return responseHelper_1.ResponseHelper.error(res, 'Stream n+�o encontrada ou n+�o pertence ao usu+�rio', 404);
        }
        // Retornar URLs do stream
        return responseHelper_1.ResponseHelper.success(res, {
            streamId: stream.id,
            streamKey: stream.streamKey,
        ...
        return responseHelper_1.ResponseHelper.error(res, 'Erro ao obter URLs da stream', 500);
    }
});
// POST /api/streams/:id/urls - Salvar configura+�+�es de URLs RTMP/SRT
// ===== ROUTE START =====
router.post('/streams/:id/urls', async (req, res) => {
    try {
        const userId = (0, auth_1.getUserIdFromToken)(req);
        const { id: streamId } = req.params;
        ...
        const updateData = {};
        if (rtmpIngestUrl !== undefined)
            updateData.rtmpIngestUrl = rtmpIngestUrl;
        if (srtIngestUrl !== undefined)
            updateData.srtIngestUrl = srtIngestUrl;
        if (playbackUrl !== undefined)
            updateData.playbackUrl = playbackUrl;
        if (streamKey !== undefined)
            updateData.streamKey = streamKey;
        const updatedStream = await index_1.Streamer.findOneAndUpdate({ id: streamId }, updateData, { returnDocument: 'after' });
        ...
        responseHelper_1.ResponseHelper.error(res, 'Falha ao salvar configura+�+�es de URLs', 500);
    }
});
// ===== ROUTE START =====
router.post('/streams/:id/save', async (req, res) => {
    try {
        ...
        const stream = await index_1.Streamer.findOneAndUpdate({ id: req.params.id }, { $set: updateData }, { returnDocument: 'after' });
        // 🪝 Webhook LiveGo: sala atualizada (info) + metadados
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterUpdateRoomInfo', { RoomId: req.params.id, UpdatedFields: Object.keys(updateData), UpdateTime: Date.now() });
            for (const f of Object.keys(updateData)) {
                const v = updateData[f];
                if (v === '' || v === null || v === undefined) {
                    (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterDelMetadata', { RoomId: req.params.id, MetadataKeys: [f], EventTime: Date.now() });
                }
                else {
                    (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterSetMetadata', { RoomId: req.params.id, Metadata: [{ Key: f, Value: typeof v === 'object' ? JSON.stringify(v) : String(v) }], EventTime: Date.now() });
                }
            }
        }
        catch (e) {
            console.warn('[WEBHOOK] room updated', e);
        }
        if (!stream) {
...
router.post('/streams/:id/cover', async (req, res) => {
    try {
        ...
        const stream = await index_1.Streamer.findOneAndUpdate({ id: req.params.id }, { avatar: coverUrl }, { returnDocument: 'after' });
        // 🪝 Webhook LiveGo: sala atualizada (cover)
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterUpdateRoomInfo', { RoomId: req.params.id, UpdatedFields: ['coverUrl'], CoverURL: coverUrl, UpdateTime: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] cover updated', e);
        }
        if (!stream) {
            return res.status(404).json({ success: false, error: 'Stream not found' });
        }
...
router.get('/streams/:id/online-users', async (req, res) => {
    try {
        ...
        const streamDoc = await index_1.Streamer.findOne({
            $or: [{ id: streamId }, { streamKey: streamId }]
        }).select('startTime').lean();
        const sessionStart = streamDoc?.startTime ? new Date(streamDoc.startTime) : null;
        const sessionFilter = (sessionStart && !isNaN(sessionStart.getTime()))
            ? { createdAt: { $gte: sessionStart } }
            : {};
        // Buscar usu+�rios marcados como online nesta stream no banco de dados
        const onlineUsersInStream = await index_1.User.find({
            isOnline: true,
            currentStreamId: streamId,
        ...
        if (onlineUsersInStream.length === 0) {
            ...
            const { GiftTransaction } = await Promise.resolve().then(() => __importStar(require('../models')));
            ...
            if (senderIds.length > 0) {
                const senderUsers = await index_1.User.find({
                    id: { $in: senderIds },
                    name: { $exists: true, $nin: ['', null] }
                }).select('id name avatarUrl identification level activeFrameId frameExpiration');
        ...
        if (usersWithValue.length === 0) {
            ...
            if (stream && stream.hostId) {
                const host = await index_1.User.findOne({ id: stream.hostId }).select('id name avatarUrl identification level activeFrameId frameExpiration');
                if (host) {
                    const hostData = {
                        id: host.id,
                        name: host.name,
...
router.get('/streams/:streamId/join', async (req, res) => {
    try {
        ...
        const user = await index_1.User.findOneAndUpdate({ id: userId }, {
            isOnline: true,
            currentStreamId: streamId,
            lastSeen: new Date().toISOString()
...
router.post('/streams/:streamId/leave', async (req, res) => {
    try {
        ...
        const activeHostStreams = await index_1.Streamer.find({
            hostId: userId,
            isLive: true,
            id: { $ne: streamId }
        });
        ...
        await index_1.Streamer.findOneAndUpdate({ id: streamId, viewers: { $gt: 0 } }, // Apenas se viewers > 0
        { $inc: { viewers: -1 } });
        // 🪝 Webhook LiveGo: membro saiu (via REST /streams/:streamId/leave)
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterMemberStatusChange', { RoomId: streamId, UserId: userId, Status: 'offline', Action: 'leave', Timestamp: Date.now() });
        }
        catch (e) {
...
router.post('/streams/:id/toggle-mic', async (req, res) => {
    try {
        ...
        let currentEnabled;
        if (session) {
            currentEnabled = !session.isMicrophoneMuted;
        }
        else {
            const streamDoc = await index_1.Streamer.findOne({ id: streamId }).select('microphoneEnabled').lean();
            currentEnabled = (streamDoc?.microphoneEnabled) !== false;
        }
        ...
        if (session) {
            await sessions.updateOne({ streamId, endTime: { $exists: false } }, { $set: { isMicrophoneMuted: !enabled, updatedAt: new Date() } });
        }
        await index_1.Streamer.findOneAndUpdate({ id: streamId }, { $set: { microphoneEnabled: enabled } });
        const io = req.app.get('io');
        if (io) {
...
router.post('/streams/:id/toggle-sound', async (req, res) => {
    try {
        ...
        let currentEnabled;
        if (session) {
            currentEnabled = !session.isStreamMuted;
        }
        else {
            const streamDoc = await index_1.Streamer.findOne({ id: streamId }).select('soundEnabled').lean();
            currentEnabled = (streamDoc?.soundEnabled) !== false;
        }
        const desired = (req.body || {}).soundEnabled;
        const enabled = typeof desired === 'boolean' ? desired : !currentEnabled;
        if (session) {
            await sessions.updateOne({ streamId, endTime: { $exists: false } }, { $set: { isStreamMuted: !enabled, updatedAt: new Date() } });
        }
        await index_1.Streamer.findOneAndUpdate({ id: streamId }, { $set: { soundEnabled: enabled } });
        const io = req.app.get('io');
        if (io) {
...
router.post('/streams/:id/toggle-auto-follow', async (req, res) => {
    try {
        ...
        if (session) {
            currentState = !!session.isAutoFollowEnabled;
        }
        else {
            const doc = await index_1.Streamer.findOne({ id: streamId }).select('autoFollowEnabled').lean();
            currentState = !!(doc?.autoFollowEnabled);
        }
        const newState = typeof isEnabled === 'boolean' ? isEnabled : !currentState;
        if (session) {
            await sessions.updateOne({ streamId, endTime: { $exists: false } }, { $set: { isAutoFollowEnabled: newState, updatedAt: new Date() } });
        }
        await index_1.Streamer.findOneAndUpdate({ id: streamId }, { $set: { autoFollowEnabled: newState } });
        const io = req.app.get('io');
        if (io) {
...
router.post('/streams/:id/toggle-auto-invite', async (req, res) => {
    try {
        ...
        let currentState = false;
        if (session) {
            currentState = !!session.isAutoPrivateInviteEnabled;
        }
        else {
            const doc = await index_1.Streamer.findOne({ id: streamId }).select('autoInviteEnabled').lean();
            currentState = !!(doc?.autoInviteEnabled);
        }
        const newState = typeof isEnabled === 'boolean' ? isEnabled : !currentState;
        if (session) {
            await sessions.updateOne({ streamId, endTime: { $exists: false } }, { $set: { isAutoPrivateInviteEnabled: newState, updatedAt: new Date() } });
        }
        await index_1.Streamer.findOneAndUpdate({ id: streamId }, { $set: { autoInviteEnabled: newState } });
        const io = req.app.get('io');
        if (io) {
...
router.post('/streams/:id/end-session', async (req, res) => {
    try {
        ...
        const { StreamHistory } = await Promise.resolve().then(() => __importStar(require('../models')));
        ...
        let updatedStream;
        try {
            updatedStream = await index_1.Streamer.findOneAndUpdate({ id: streamId }, {
                isLive: false,
                endTime: new Date(endTime).toISOString(),
                streamStatus: 'ended',
                viewers: 0
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterDestroyRoom', { RoomId: streamId, HostId: stream.hostId || '', EndedBy: 'owner', EndTime: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] room destroyed (end session)', e);
        }
        // 5. Atualizar status do host
        let updatedUser;
        try {
            const User = await Promise.resolve().then(() => __importStar(require('../models'))).then(m => m.User);
            updatedUser = await User.findOneAndUpdate({ id: stream.hostId }, { $set: { isLive: false, isOnline: false, currentStreamId: null } }, { returnDocument: 'after' });
            if (!updatedUser) {
        ...
        try {
            const User = await Promise.resolve().then(() => __importStar(require('../models'))).then(m => m.User);
            await User.updateMany({ currentStreamId: streamId }, {
                currentStreamId: null,
                lastSeen: new Date().toISOString()
            });
        ...
        if (stream?.hostId) {
            try {
                await index_1.LiveCard.findOneAndUpdate({ hostId: stream.hostId }, { $set: {
                        isLive: false,
                        streamStatus: 'ended',
...
router.delete('/cards/:streamId', async (req, res) => {
    try {
        ...
        await index_1.Streamer.findOneAndUpdate({ id: streamId }, {
            isLive: false,
            streamStatus: 'ended',
...
router.post('/streams/:id/gift', async (req, res) => {
    try {
        ...
        const U = await Promise.resolve().then(() => __importStar(require('../models'))).then(m => m.User);
        const G = await Promise.resolve().then(() => __importStar(require('../models'))).then(m => m.Gift);
        const sender = await U.findOne({ id: fromUserId });
        if (!sender)
            return res.status(404).json({ error: 'Sender not found' });
        // Find stream to get receiver
        const stream = await index_1.Streamer.findOne({ id: req.params.id });
        if (!stream)
            return res.status(404).json({ error: 'Stream not found' });
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterGift', { RoomId: req.params.id, FromUserId: fromUserId, FromUserName: sender.name || '', ToUserId: stream.hostId, ToUserName: stream.name || '', GiftName: giftName, GiftPrice: price, Quantity: amount || 1, TotalValue: totalValue, Timestamp: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] gift (liveRoutes)', e);
        }
        let updatedSender;
        let updatedReceiver;
        if (fromUserId === stream.hostId) {
            ...
            updatedSender = await U.findOneAndUpdate({ id: fromUserId }, { $inc: { diamonds: -totalValue, enviados: totalValue, receptores: totalValue, earnings: totalValue } }, { returnDocument: 'after' });
            ...
            updatedSender = await U.findOneAndUpdate({ id: fromUserId }, { $inc: { diamonds: -totalValue, enviados: totalValue } }, { returnDocument: 'after' });
            if (stream.hostId) {
                updatedReceiver = await U.findOneAndUpdate({ id: stream.hostId }, { $inc: { receptores: totalValue, earnings: totalValue } }, { returnDocument: 'after' });
        ...
        await index_1.Streamer.findOneAndUpdate({ id: req.params.id }, {
        ...
        await index_1.GiftTransaction.create([{
        ...
        if (stream.autoFollowEnabled && stream.hostId && String(stream.hostId) !== String(fromUserId)) {
            try {
                const hostId = String(stream.hostId);
                const gifterId = String(fromUserId);
                const existingFollow = await index_1.Followers.findOne({ followerId: hostId, followingId: gifterId, isActive: true });
                if (!existingFollow) {
                    const inactiveFollow = await index_1.Followers.findOne({ followerId: hostId, followingId: gifterId, isActive: false });
                    if (inactiveFollow) {
                        await index_1.Followers.findOneAndUpdate({ followerId: hostId, followingId: gifterId, isActive: false }, { $set: { isActive: true, followedAt: new Date() }, $unset: { unfollowedAt: 1 } });
                    }
                    else {
                        await index_1.Followers.create({
                    ...
                    await index_1.User.findOneAndUpdate({ id: hostId }, { $inc: { following: 1 }, $addToSet: { followingList: gifterId } });
                    await index_1.User.findOneAndUpdate({ id: gifterId }, { $inc: { fans: 1 }, $addToSet: { followersList: hostId }, $set: { isFollowed: true } });
                    // Avisa em tempo real que o host passou a seguir o remetente
                    const ioAuto = req.app.get('io') || global.io;
                    if (ioAuto) {
                    ...
                    try {
                        (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterFollow', { FollowerId: hostId, FollowedId: gifterId, AutoFollow: true, Timestamp: Date.now() });
                    }
                    catch (e) {
                        console.warn('[WEBHOOK] auto-follow', e);
                    }
...
router.put('/streams/:id/quality', async (req, res) => {
    try {
        ...
        await index_1.Streamer.updateOne({ id: streamId }, { quality: quality });
    ...
    try {
        ...
        if (user.isLive) {
            ...
            if (activeStream) {
                ...
                await index_1.User.findOneAndUpdate({ id: userId }, { $set: { isLive: false } });
            }
        }
        const streamId = userId;
        const streamKey = 'stream_' + (0, uuid_1.v4)();
        ...
        await index_1.Streamer.findOneAndUpdate({ id: streamId }, { $set: { id: streamId, hostId: userId, name: user.name || userId, isLive: false, streamStatus: 'preparing', startTime: new Date(), streamKey: streamKey, liveId: liveId, pushUrl: pushUrl, title: title, category: finalCategory, country: finalCountry } }, { upsert: true, returnDocument: 'after' });
        n; //
...
router.get('/lives/:id', async (req, res) => {
    try {
        ...
        const streamer = await index_1.Streamer.findOne({ id }).lean();
        if (!streamer) {
...
router.post('/lives/:id/end', async (req, res) => {
    try {
        ...
        await index_1.Streamer.findOneAndUpdate({ id: realId }, { $set: {
                isLive: false,
                streamStatus: 'ended',
                endTime: new Date()
            } });
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterDestroyRoom', { RoomId: realId, HostId: userId || '', EndedBy: 'owner', EndTime: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] room destroyed (lives end)', e);
        }
        // Atualizar status do usu+rio
        await index_1.User.findOneAndUpdate({ id: userId }, { $set: {
                isLive: false,
        ...
        return (0, responseHelpers_1.internalServerErrorResponse)(res, 'Erro ao encerrar stream', error);
    }
});
...
router.get('/live/nearby', async (req, res) => {
    try {
        ...
        const nearbyStreams = await index_1.Streamer.find({
            isLive: true,
            streamStatus: 'active',
            name: { $exists: true, $nin: ['', null] },
        ...
        const protectedActiveStreams = (0, srsStreamMapper_1.mapStreamsToProtectedArrayFlexible)(validStreams);
...
router.get('/live/following', async (req, res) => {
    try {
        ...
        const followingStreams = await index_1.Streamer.find({
            isLive: true,
            streamStatus: 'active',
...
router.get('/live/new', async (req, res) => {
    try {
        ...
        const newStreams = await index_1.Streamer.find({
            isLive: true,
            streamStatus: 'active',
            name: { $exists: true, $nin: ['', null] },
...
router.post('/streams/:id/like', async (req, res) => {
    try {
        ...
        const existingLike = await index_1.StreamLike.findOne({ streamId, userId });
        if (existingLike) {
            return res.status(400).json({ error: 'Already liked' });
        }
        // Criar novo like
        const like = await index_1.StreamLike.create({
            streamId,
            userId,
            timestamp: new Date().toISOString()
        });
        // Incrementar contador de likes da stream
        await index_1.Streamer.updateOne({ id: streamId }, { $inc: { likes: 1 } });
        // Obter contagem atualizada
        const updatedStream = await index_1.Streamer.findOne({ id: streamId });
...
// ===== ROUTE START =====
router.delete('/streams/:id/like', async (req, res) => {
    try {
        // Usar ID real diretamente
        const streamId = req.params.id;
        const userId = req.body.userId;
        ...
        const like = await index_1.StreamLike.findOne({ streamId, userId });
        if (!like) {
            return res.status(404).json({ error: 'Like not found' });
        }
        // Remover like
        await index_1.StreamLike.deleteOne({ streamId, userId });
...
router.post('/streams/:streamId/end', async (req, res) => {
    try {
        ...
        const otherActiveStreams = await index_1.Streamer.find({
            hostId: stream.hostId,
            isLive: true,
            id: { $ne: streamId }
        });
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterDestroyRoom', { RoomId: streamId, HostId: stream.hostId || '', EndedBy: isOwner ? 'owner' : 'admin', EndTime: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] room destroyed (end)', e);
        }
...
// ===== ROUTE START =====
router.post('/streams/end-all', async (req, res) => {
    try {
        const userId = (0, auth_1.getUserIdFromToken)(req);
        if (!userId) {
            return res.status(401).json({
        ...
        await index_1.Streamer.updateMany({ hostId: userId, isLive: true }, {
            isLive: false,
            streamStatus: 'ended',
            endTime: new Date(),
            endedBy: 'owner_force',
...
// ===== ROUTE START =====
router.post('/streams/:streamId/heartbeat', async (req, res) => {
    try {
        const userId = (0, auth_1.getUserIdFromToken)(req);
        const { streamId } = req.params;
        if (!userId) {
        ...
        // }
        await index_1.Streamer.findOneAndUpdate({ id: streamId }, updateData);
        res.json({
            success: true,
...
// ===== ROUTE START =====
router.post('/admin/streams/:streamId/force-end', async (req, res) => {
    try {
        const userId = (0, auth_1.getUserIdFromToken)(req);
        const { streamId } = req.params;
...
// ===== ROUTE START =====
router.get('/admin/streams/zombie-stats', async (req, res) => {
    try {
        const userId = (0, auth_1.getUserIdFromToken)(req);
        if (!userId) {
            return res.status(401).json({
...
// ===== ROUTE START =====
router.post('/admin/streams/cleanup-zombies', async (req, res) => {
    try {
        const userId = (0, auth_1.getUserIdFromToken)(req);
        if (!userId) {
            return res.status(401).json({
...
router.post('/streams/prepare', async (req, res) => {
    try {
        ...
        const newStream = await index_1.Streamer.create({
            id: streamId,
            hostId: userId,
            name: user.name,
            avatar: user.avatarUrl || '',
...
router.post('/streams/:id/start', async (req, res) => {
    try {
        ...
        const { UserStatus } = await Promise.resolve().then(() => __importStar(require('../models')));
        await UserStatus.findOneAndUpdate({ userId: userId }, {
            isOnline: true,
            lastSeen: new Date(),
            updatedAt: new Date()
        }, { upsert: true, returnDocument: 'after' });
...
router.post('/stark/live/end', async (req, res) => {
    try {
        ...
        // Atualizar LiveCard para ended
        try {
            await index_1.LiveCard.findOneAndUpdate({ hostId: tokenUserId }, { $set: {
                    isLive: false,
                    streamStatus: 'ended',
                    endTime: endTime,
                    updatedAt: endTime
...
router.post('/streams/:id/live-message', async (req, res) => {
    try {
        ...
        const stream = await index_1.Streamer.findOne({ id }).lean();
        if (!stream) {
...
router.get("/streams/:id/live-messages", async (req, res) => {
    try {
        ...
        const messages = await index_1.LiveMessage.find({ streamId: id })
            .sort({ timestamp: 1 })
            .limit(limit)
            .lean();
        res.json({ success: true, messages });
...
router.post("/streams/:id/publish", async (req, res) => {
    try {
        ...
        const stream = await index_1.Streamer.findOneAndUpdate({ id: realId, hostId: userId }, { $set: streamerData }, { upsert: true, new: true });
        await index_1.User.findOneAndUpdate({ id: userId }, { $set: { isLive: true, currentStreamId: realId, isOnline: true } });
        await index_1.LiveCard.findOneAndUpdate({ hostId: userId }, {
            $set: {
                hostId: userId,
                name: stream.name,
                avatar: stream.avatar,
...
router.post('/streams/:id/transfer-owner', async (req, res) => {
    try {
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterOwnerChange', { RoomId: id, OldOwnerId: oldOwnerId, NewOwnerId: newOwnerId, Timestamp: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] owner change', e);
        }
