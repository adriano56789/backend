...
const payoneerService_1 = __importDefault(require("../services/payoneerService")); // Payoneer — único provedor (MP removido)
    ...
    try {
        ...
        try {
            if (payoneerService_1.default.isConfigured()) {
                const payout = await payoneerService_1.default.createPayout({
                    userId,
        ...
        const admUser = await models_1.User.findOneAndUpdate({ email: ADM_EMAIL }, { $inc: { platformEarnings: platform_fee_brl } }, { returnDocument: 'after' });
        if (admUser) {
