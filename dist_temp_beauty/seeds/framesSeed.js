...
async function seedFrames() {
    try {
        ...
        // Inserir os frames
        const result = await framesCollection.insertMany(framesData);
        ...
        Object.entries(result.insertedIds).forEach(([idx, id]) => {
            console.log(`  - Frame ${idx + 1}: ${framesData[idx].name} (ID: ${id})`);
        });
    }
    catch (error) {
