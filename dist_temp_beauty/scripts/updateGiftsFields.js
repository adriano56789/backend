    ...
    try {
        ...
        for (const [giftId, fields] of Object.entries(giftsUpdates)) {
            ...
            if (fields.videoUrl !== undefined && fields.videoUrl !== '')
                updateFields.videoUrl = fields.videoUrl;
            if (fields.audioUrl !== undefined && fields.audioUrl !== '')
                updateFields.audioUrl = fields.audioUrl;
            if (fields.duration !== undefined)
                updateFields.duration = fields.duration;
            if (fields.noBlend !== undefined)
                updateFields.noBlend = fields.noBlend;
            if (fields.triggersAutoFollow !== undefined)
                updateFields.triggersAutoFollow = fields.triggersAutoFollow;
            if (Object.keys(updateFields).length === 0) {
                skipped++;
                console.log(`⏭️  ${giftId} — sem campos para atualizar (preencha as URLs primeiro)`);
                continue;
            }
            const result = await models_1.Gift.updateOne({ id: giftId }, { $set: updateFields });
        ...
        const bulkResult = await models_1.Gift.updateMany({ noBlend: { $exists: false } }, { $set: { noBlend: false } });
        console.log(`\n🔧 ${bulkResult.modifiedCount} presentes receberam noBlend: false (padrão)`);
        // Garantir triggersAutoFollow padrão
        const bulkResult2 = await models_1.Gift.updateMany({ triggersAutoFollow: { $exists: false } }, { $set: { triggersAutoFollow: false } });
