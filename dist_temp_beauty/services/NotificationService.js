...
class NotificationService {
    ...
    static async sendFcm(userId, fcm) {
        try {
            ...
            if (tokenList.length > 0) {
                await (0, webPushService_1.sendPushNotificationToMultiple)(tokenList, {
                    title: fcm.title,
                    body: fcm.body,
                    data: fcm.data || {},
                    image: fcm.image,
     ...
     */
    static async notifyLiveStartedToAll(hostId, hostName, hostAvatar, streamId, streamTitle) {
        const message = `${hostName || 'Alguém'} está ao vivo!`;
        try {
            // Buscar TODOS os device tokens (não apenas de seguidores)
            const allTokens = await index_1.DeviceToken.find({}).select('token userId').lean();
            ...
            for (let i = 0; i < tokenList.length; i += CHUNK_SIZE) {
                ...
                await (0, webPushService_1.sendPushNotificationToMultiple)(chunk, {
                    title: hostName || 'LiveGO',
    ...
    static async notifyNewMessage(io, toUserId, fromUserId, fromUserName, preview, conversationId, fromUserAvatar) {
