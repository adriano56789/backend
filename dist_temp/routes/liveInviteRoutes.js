...
router.post('/role', async (req, res) => {
    try {
        ...
        try {
            ...
            (0, WebhookBroadcasterService_1.emitWebhook)(roleEvent, { RoomId: streamId, UserId: uid, Role: status, Timestamp: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] role change', e);
        }
...
router.post('/invite', async (req, res) => {
    try {
        ...
        const [inviterUser, inviteeUser] = await Promise.all([
            models_1.User.findOne({ id: inviterUsername }).select('name avatarUrl').lean(),
            models_1.User.findOne({ id: inviteeUsername }).select('name avatarUrl').lean()
        ]);
        const resolvedInviterName = inviterName || inviterUser?.name || inviterUsername;
        const resolvedInviteeName = inviteeName || inviteeUser?.name || inviteeUsername;
        const inviterAvatar = inviterUser?.avatarUrl || '';
        ...
        const existing = await LiveInvite_1.LiveInvite.findOne({
            inviteType,
            status: 'pending',
            $or: [
        ...
        try {
            ...
            await NotificationService.notifyLiveInvite(io, inviteeUsername, inviterUsername, resolvedInviterName, inviteType, invite._id.toString(), streamId);
        }
        catch (notifErr) {
...
router.post('/invite/respond', async (req, res) => {
    try {
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterSeatListChange', { RoomId: invite.streamId || '', InviteId: inviteId, Action: action, InviteType: invite.inviteType || '', Inviter: invite.inviterUsername, Invitee: invite.inviteeUsername, Timestamp: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] seat change', e);
        }
        const io = req.app.get('io');
        if (io) {
...
router.post('/co-host/exit', async (req, res) => {
    try {
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterSeatListChange', { RoomId: streamId, UserId: userId, Action: 'exit', InviteType: 'co-host', Timestamp: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] seat change (co-host exit)', e);
        }
        const io = req.app.get('io');
        if (io) {
...
router.post('/battle/exit', async (req, res) => {
    try {
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterSeatListChange', { RoomId: streamId, UserId: userId, Action: 'exit', InviteType: 'pk-battle', Timestamp: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] seat change (battle exit)', e);
        }
        if (battleId) {
            try {
