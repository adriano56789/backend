...
router.get('/:id/messages', async (req, res) => {
    try {
        ...
        const senders = await index_1.User.find({
            id: { $in: senderIds }
        }).select('id name avatarUrl');
...
router.post('/send', async (req, res) => {
    try {
        ...
        const permCheck = await (0, chatPermission_1.canSendMessage)(from, to);
        ...
        const newMessage = (await index_1.ChatMessage.findOneAndUpdate({ id: messageId }, {
            $set: {
                id: messageId,
                conversationId,
                senderId: from,
        ...
        (0, activityHelpers_1.pushRecentActivity)(from, {
            action: 'chat_message_sent',
            resource: 'chat',
            endpoint: '/api/chats/send'
        });
        // FIX: Include sender info so recipient sees name/avatar immediately in real-time
        const sender = await index_1.User.findOne({ id: from }).select('id name avatarUrl level birthday').lean();
        ...
        try {
            ...
            await NotificationService.notifyNewMessage(io, to, from, senderName, preview, conversationId, sender?.avatarUrl);
        }
        catch (notifErr) {
...
router.post('/:id/messages', async (req, res) => {
    try {
        ...
        await index_1.Chat.findOneAndUpdate({ id }, {
            $set: {
                lastMessage: {
        ...
        try {
            ...
            if (receiverIds.length > 0) {
                const sender = await index_1.User.findOne({ id: senderId }).select('name avatarUrl');
                const senderName = sender?.name || 'Alguém';
                const preview = messageType === 'image' ? '[Imagem]' : content;
                for (const receiverId of receiverIds) {
                    await NotificationService.notifyNewMessage(io, receiverId, senderId, senderName, preview, id, sender?.avatarUrl);
                }
            }
        }
        catch (notifErr) {
...
router.put('/messages/:id/read', async (req, res) => {
    try {
        ...
        await index_1.ChatMessage.findOneAndUpdate({ id }, {
            $set: {
                isRead: true,
                readAt: new Date()
            }
