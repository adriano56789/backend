...
exports.sendRouletteStateToClient = sendRouletteStateToClient;
...
async function sendRouletteStateToClient(socket, ownerId) {
    try {
        const items = await (0, RouletteItem_1.findActiveByOwner)(ownerId);
        const userDoc = await models_1.User.findOne({ id: ownerId }).exec();
        const spinCost = userDoc && Number(userDoc.rouletteSpinCost) > 0 ? Number(userDoc.rouletteSpinCost) : 0;
    ...
    try {
        ...
        if (cost > 0 && ownerId) {
            try {
                ...
                if (streamId && streamId !== 'unknown') {
                    await models_1.Streamer.findOneAndUpdate({ id: streamId }, { $inc: { diamonds: cost } }, { upsert: true }).exec();
                    ...
                    try {
                        ...
                        await incrementCoins(db.collection('streamsessions'), streamId, cost);
                    }
                    catch (sessionErr) {
                ...
                await models_1.User.findOneAndUpdate({ id: ownerId }, {
        ...
        if (cost > 0) {
            try {
                ...
                const spinnerDoc = await models_1.User.findOne({ id: spinningUserId }).select('diamonds enviados').lean();
                if (spinnerDoc) {
                    io.to(`user_${spinningUserId}`).emit('diamonds_updated', {
                        userId: spinningUserId,
                        diamonds: spinnerDoc.diamonds,
                        enviados: spinnerDoc.enviados,
                        change: -cost,
                        timestamp: new Date().toISOString(),
                ...
                const hostDoc = await models_1.User.findOne({ id: ownerId }).select('earnings receptores').lean();
                io.emit('earnings_updated', {
                    userId: ownerId,
                    diamonds: cost,
                    totalEarnings: Number(hostDoc?.earnings ?? 0),
                    receptores: Number(hostDoc?.receptores ?? 0),
                    timestamp: new Date().toISOString(),
                    source: 'roulette_spin',
                ...
                if (streamId && streamId !== 'unknown') {
                    const updatedStream = await models_1.Streamer.findOne({ id: streamId }).select('diamonds').lean();
                    io.emit('live_coins_updated', {
                        streamId,
                        coins: cost,
                        totalCoins: updatedStream?.diamonds || 0,
                        timestamp: new Date().toISOString(),
                        fromUser: spinningUserId,
                        giftName: 'Roleta'
                    });
