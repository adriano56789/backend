...
router.post('/request', async (req, res) => {
    try {
        const guestId = (0, auth_1.getUserIdFromToken)(req);
        if (!guestId)
        ...
        const stream = await models_1.Streamer.findOne({ id: streamId, isLive: true, streamStatus: 'active' });
        if (!stream) {
        ...
        try {
            ...
            await NotificationService.notifyCallInvitation(io, hostId, guestId, guest.name || guestId, invitationId, invitation.roomId, invitation.streamId);
        }
        catch (notifErr) {
