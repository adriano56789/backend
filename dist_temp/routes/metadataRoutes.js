...
router.get('/ranking/:period', async (req, res) => {
    try {
                ...
                if (session && session.giftsReceived > 0) {
                    ...
                    if (streamer) {
                        ...
                        liveRanking.push({
                            ...
                            level: streamer.level || 1,
                            gender: streamer.gender || 'not_specified',
                            age: streamer.age || 0,
        ...
        if (scope === 'fans' && targetUserId && typeof targetUserId === 'string') {
            ...
            const fanTransactions = await models_1.GiftTransaction.find({
                toUserId: targetUserId,
                fromUserId: { $ne: targetUserId },
                ...dateQuery,
            }).limit(10000).lean();
            const senderTotals = new Map();
            fanTransactions.forEach((tx) => {
            ...
            const senders = await models_1.User.find({ id: { $in: senderIds } }).lean();
