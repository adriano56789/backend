...
router.post('/streams/:id/private-invite', async (req, res) => {
    try {
        ...
        await models_1.Streamer.updateOne({ id: streamId }, { $addToSet: { invitedUsers: userId } });
        ...
        await models_1.LiveCard.updateOne({ $or: [{ hostId: streamId }, { streamKey: streamId }] }, { $addToSet: { invitedUsers: userId } }).catch(() => { });
        ...
        await Promise.all([
            (0, activityHelpers_1.pushRecentActivity)(stream.hostId, {
...
router.get('/streams/invited-streams', async (req, res) => {
    try {
        ...
        const invited = await models_1.Streamer.find({
            invitedUsers: userId,
            isLive: true
        }).select('id streamKey hostId').lean();
        const streamIds = Array.from(new Set(invited.map((s) => s.id || s.streamKey || s.hostId).filter(Boolean)));
...
router.get('/streams/:id/access-check', async (req, res) => {
    try {
        ...
        await (0, activityHelpers_1.pushRecentActivity)(requestingUser, {
        ...
        if (requestingUser === stream.hostId) {
            ...
            const invitedUsers = Array.isArray(stream.invitedUsers) ? stream.invitedUsers : [];
            ...
            if (!invitedUsers.includes(requestingUser)) {
                ...
                const followerCheck = settings.followersOnly
                    ? await models_1.Followers.findOne({
                ...
                const friendCheck = settings.friendsOnly
                    ? await models_1.Friendship.findOne({
                        $or: [
...
router.post('/friends/invite', async (req, res) => {
    try {
        ...
        await Promise.all([
            (0, activityHelpers_1.pushRecentActivity)(fromUserId, {
                action: 'friend_invite_sent',
                resource: 'social_interaction',
                endpoint: '/api/interactions/friends/invite'
            }),
            (0, activityHelpers_1.pushRecentActivity)(toUserId, {
...
router.get('/feed/photos', async (req, res) => {
    try {
        ...
        const [userPhotoFeed, videoFeed, usersWithObras] = await Promise.all([
            ...
            models_1.User.find({ ...userObrasFilter }).select('id obras').limit(50).lean()
        ]);
        // Fallback: usuários SEM obras → usar galeria ProfilePhoto (igual à aba Obras)
        const emptyObrasIds = usersWithObras
            .filter((u) => !Array.isArray(u.obras) || u.obras.length === 0)
            .map((u) => u.id);
        const galleryFallback = emptyObrasIds.length > 0
            ? await models_1.ProfilePhoto.find({ userId: { $in: emptyObrasIds }, isActive: true, photoType: 'gallery' })
                .sort({ createdAt: -1 }).limit(30).lean()
            : [];
        const obrasFeed = usersWithObras.flatMap((u) => (Array.isArray(u.obras) ? u.obras : []).filter((o) => o && o.url).map((o) => ({
            id: o.id,
            userId: u.id,
            photoUrl: o.url,
...
router.post('/streams/:id/kick', appOwnerProtection_1.kickProtection, async (req, res) => {
    try {
        ...
        await Streamer.updateOne({ id: streamId }, { $addToSet: { kickedUsers: String(userId) } });
