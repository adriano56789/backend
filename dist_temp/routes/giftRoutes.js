...
async function processGiftSend(fromUserId, toUserId, giftId, quantity, streamId, io) {
    try {
        ...
        await (0, activityHelpers_1.pushRecentActivity)(fromUserId, {
            action: 'live_gift',
        ...
        try {
            (0, WebhookBroadcasterService_1.emitWebhook)('LiveGo.CallbackAfterGift', { RoomId: streamId || 'unknown', FromUserId: fromUserId, FromUserName: fromUser.name || '', ToUserId: toUserId, ToUserName: toUser.name || '', GiftId: gift.id, GiftName: gift.name || '', GiftPrice: giftPrice, Quantity: quantity, TotalValue: totalCost, Timestamp: Date.now() });
        }
        catch (e) {
            console.warn('[WEBHOOK] gift (giftRoutes)', e);
        }
        ...
        try {
            ...
            await NotificationService.notifyGiftReceived(io, toUserId, fromUserId, fromUser.name || 'Alguém', gift.id, gift.name || 'Presente', gift.icon || '', quantity, totalCost, streamId);
        }
        catch (notifErr) {
