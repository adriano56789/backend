...
class FraudDetectionMiddleware {
    static async detectFraud(req, res, next) {
        try {
            ...
            const ipBanned = await BannedEntity_1.BannedEntity.findOne({
                entityType: 'ip',
                entityId: clientIp,
                active: true,
                $or: [
            ...
            if (deviceFingerprint) {
                const deviceBanned = await BannedEntity_1.BannedEntity.findOne({
                    entityType: 'device',
            ...
            if (userId) {
                const userBanned = await BannedEntity_1.BannedEntity.findOne({
                    entityType: 'user',
                    entityId: userId,
                    active: true,
                    $or: [
                ...
                if (user?.email) {
                    const emailBanned = await BannedEntity_1.BannedEntity.findOne({
                        entityType: 'email',
    ...
    static async banEntity(entityType, entityId, reason, evidence, permanent = true) {
        try {
            // Verificar se já existe banimento ativo
            const existingBan = await BannedEntity_1.BannedEntity.findOne({
                entityType,
                entityId,
                active: true
            });
            ...
            const ban = await BannedEntity_1.BannedEntity.create({
                entityType,
                entityId,
                reason,
                evidence,
    ...
    static async banRelatedEntities(ip, deviceFingerprint, userId, userEmail, reason, evidence) {
        try {
            const bans = [];
            // Banir IP
            if (ip) {
            ...
            if (mainBan && ip && deviceFingerprint && userId && userEmail) {
                await BannedEntity_1.BannedEntity.updateOne({ _id: mainBan._id }, {
                    $set: {
                        relatedEntities: {
                            ips: [ip],
...
exports.default = FraudDetectionMiddleware;
