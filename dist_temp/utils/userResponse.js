...
exports.standardizeUserResponse = standardizeUserResponse;
exports.standardizeUsersList = standardizeUsersList;
const idHelper_1 = require("./idHelper");
/**
 ...
 */
function standardizeUserResponse(user, viewerId) {
    if (!user) {
    ...
    const isOwnView = !!viewerId && (0, idHelper_1.getRealUserId)(user) && viewerId === (0, idHelper_1.getRealUserId)(user);
    const hideActivity = !isOwnView && user.showActivityStatus === false;
    const hideLocation = !isOwnView && user.showLocation === false;
    return {
        id: (0, idHelper_1.getRealUserId)(user),
        name: user.name || "",
        ...
        fans: Math.max(0, user.fans || 0),
        following: Math.max(0, user.following || 0),
        ...
        cameraPermissionStatus: user.cameraPermissionStatus || 'prompt',
        cameraAccessEnabled: user.cameraAccessEnabled === true || user.cameraPermissionStatus === 'granted',
 ...
 */
function standardizeUsersList(users, viewerId) {
