...
class UserLevelService {
    ...
    static async getUserLevel(userId) {
        try {
            ...
            if (!userLevel) {
                userLevel = await UserLevel_1.UserLevel.create({
                    userId,
                    currentLevel: 1,
                    currentExp: 0,
                    totalExp: 0,
            ...
            const levelInfo = userLevel.getLevelInfo();
    ...
    static async addExp(data) {
        try {
            ...
            const result = await userLevel.addExp(amount, reason || 'EXP gained');
            // Atualizar nível no usuário principal também
            await User_1.User.findOneAndUpdate({ id: userId }, {
                $set: {
    ...
    static async addMultipleExp(data) {
        try {
            ...
            for (const gain of expGains) {
                if (gain.amount > 0) {
                    const result = await userLevel.addExp(gain.amount, gain.reason || 'EXP gained');
    ...
    static async getLeaderboard(options = {}) {
        try {
            ...
            const leaderboard = await UserLevel_1.UserLevel.find(query)
     ...
     */
    static async calculateExpForLevel(targetLevel) {
        try {
            const expRequired = UserLevel_1.UserLevel.calculateExpForLevel(targetLevel);
            // Calcular EXP total necessária para chegar até este nível
            let totalExpNeeded = 0;
            for (let i = 1; i < targetLevel; i++) {
                totalExpNeeded += UserLevel_1.UserLevel.calculateExpForLevel(i);
            }
    ...
    static async getLevelDistribution() {
        try {
            const distribution = await UserLevel_1.UserLevel.getLevelDistribution();
     ...
     */
    static async recordStreamActivity(userId, streamId, activityType, data) {
        try {
     ...
     */
    static async canLevelUp(userId) {
        try {
            const userLevel = await UserLevel_1.UserLevel.findOne({ userId });
            if (!userLevel) {
                return false;
    ...
    static async getSystemStats() {
        try {
            const totalUsers = await UserLevel_1.UserLevel.countDocuments();
            const averageLevelResult = await UserLevel_1.UserLevel.aggregate([
                { $group: { _id: null, avgLevel: { $avg: '$currentLevel' } } }
            ]);
            const averageLevel = averageLevelResult;
            const topLevel = await UserLevel_1.UserLevel.findOne({}).sort({ currentLevel: -1 });
            const totalExpDistributedResult = await UserLevel_1.UserLevel.aggregate([
