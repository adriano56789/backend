...
export class WebhookService {
  private static livekitClient: RoomServiceClient | null = null;
  ...
  private static getLiveKitClient(): RoomServiceClient {
    if (!this.livekitClient) {
      ...
      if (!apiKey || !apiSecret) {
        ...
        return null as unknown as RoomServiceClient;
      }

      try {
        this.livekitClient = new RoomServiceClient(livekitHost, apiKey, apiSecret);
    ...
    return this.livekitClient as RoomServiceClient;
  }
   ...
   */
  private static async broadcastToLiveKit(roomName: string, eventType: string, data: any): Promise<void> {
    ...
    try {
      ...
      await client.sendData(fullRoomName, bytes, DataPacket_Kind.RELIABLE, { topic: 'livechat' });
  ...
  public static async handleEvent(
    ...
    const { users, streamers, chatMessages, giftTransactions, saveCollection, startFfmpegStream, stopFfmpegStream } = context;
