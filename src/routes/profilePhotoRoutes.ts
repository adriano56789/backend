...
router.put('/:userId/photos/:obraId/set-main', async (req, res) => {
    try {
        ...
        const photo = await ProfilePhoto.findOne({ obraId: obraId, userId, photoType: 'avatar' });
...
router.delete('/me/photos/:obraId', protect, async (req: AuthRequest, res) => {
    try {
        const userId = getCurrentUserId(req);
        ...
        const photo = await ProfilePhoto.findOne({ obraId: obraId, userId });
...
router.put('/me/photos/:obraId/set-main', protect, async (req: AuthRequest, res) => {
    try {
        const userId = getCurrentUserId(req);
        const { obraId } = req.params;
...
router.put('/:userId/photos/reorder', async (req, res) => {
    try {
        ...
        await ProfilePhoto.bulkWrite(bulkOps);
