...
router.post('/invite', async (req, res) => {
  try {
    ...
    try {
      if (!(await roomExists(livekitRoom))) {
    ...
    let hostToken = '';
    try {
      hostToken = await generateLiveKitToken(
        hostId,
        livekitRoom,
        JSON.stringify({ name: req.user?.name || 'Host', type: 'call', invitationId, role: 'host' }),
        { canPublish: true }
      );
...
router.post('/respond', async (req, res) => {
  try {
    ...
    if (response === 'accept' && io) {
      ...
      try {
        if (!(await roomExists(livekitRoom))) {
          await roomService.createRoom({ name: livekitRoom, emptyTimeout: 600, maxParticipants: 10 });
        }
      } catch (_) {}
      ...
      try {
        if (!hostToken) {
          hostToken = await generateLiveKitToken(
            invitation.hostId,
            livekitRoom,
            JSON.stringify({ name: invitation.hostName, type: 'call', invitationId, role: 'host' }),
            { canPublish: true }
          );
          invitation.hostLiveKitToken = hostToken;
        }
        guestToken = await generateLiveKitToken(
          invitation.guestId,
          livekitRoom,
          JSON.stringify({ name: invitation.guestName, type: 'call', invitationId, role: 'guest' }),
          { canPublish: true }
        );
