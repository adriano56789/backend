...
async function processGiftSend(fromUserId: string, toUserId: string, giftId: string, quantity: number, streamId: string, io: any) {
    try {
        ...
        if (streamId && streamId !== 'unknown') {
            try {
                ...
                if (activeBattle) {
                    ...
                    if (io) {
                        const updated = await Battle.findById(activeBattle._id).select('scoreA scoreB');
        ...
        if (gift.triggersAutoFollow && streamId && streamId !== 'unknown' && fromUserId !== toUserId) {
          try {
            const alreadyFollows = await isFollowing(fromUserId, toUserId);
            if (!alreadyFollows) {
              await createFollow(fromUserId, toUserId);
        ...
        let comboResult: { repeatCount: number; repeatEnd: boolean; comboCount: number; groupId: string; multiplier: number } | null = null;
        if (streamId && streamId !== 'unknown') {
          comboResult = ComboService.processCombo(
            streamId, fromUserId, fromUser.name, fromUser.avatarUrl,
            gift.id, gift.name, totalCost, io,
          );
          GiftRankingService.record(
...
router.post('/streams/:streamId/gift', async (req, res) => {
  try {
    ...
    const gift = await Gift.findOne({ name: giftName }).lean();
    if (!gift) {
    ...
    const stream = await Streamer.findOne({ id: streamId }).lean();
    if (!stream) {
