...
router.post('/token/user/online/infos', async (req, res) => {
    ...
    try {
        ...
        await User.findOneAndUpdate(
            ...
            { id: userId }, // Usar o campo 'id' em vez de '_id'
...
router.post('/live/start', async (req, res) => {
    ...
    try {
        ...
        console.log(`[LIVE-START] Live iniciada: ${streamId} para usuário ${userId}`);

        res.json({
            success: true,
            message: 'Live preparada com sucesso',
            stream: {
                id: streamId,
                liveId: liveId,
                title: title,
        ...
        console.error('[LIVE-START] Erro:', error);
        res.status(500).json({ 
            success: false,
...
// Endpoint POST /api/live/end - Controla o fim da live no backend
...
router.get('/live/:category', async (req, res) => {
    ...
    try {
        ...
        if (category === 'global' || category === 'popular') {
            ...
            // Aplicar proteção máxima aos dados transformados (converter para tipo compatível)
        ...
        // Retornar streams da categoria COM PROTECAO MAXIMA - SEM IDs REAIS usando mapper flexível
...
router.post('/streams', async (req, res) => {
    try {
        ...
        const streamId = hostId; // Reutiliza ID do host para stream única
...
// GET /api/streams/:id/urls - Obter configurações de URLs de um stream
router.get('/streams/:id/urls', async (req: express.Request, res: express.Response) => {
    try {
        const userId = getUserIdFromToken(req);
        const { id: streamId } = req.params;

        // Verificar se a stream pertence ao usuário
        const stream = await Streamer.findOne({ id: streamId, hostId: userId });
        
        if (!stream) {
            return ResponseHelper.error(res, 'Stream não encontrada ou não pertence ao usuário', 404);
        }
...
// POST /api/streams/:id/urls - Salvar configurações de URLs RTMP/SRT
router.post('/streams/:id/urls', async (req: express.Request, res: express.Response) => {
    try {
        ...
        console.log(`🔗 [URLS] Configurações atualizadas para stream ${streamId} pelo usuário ${userId}`);

        ResponseHelper.success(res, {
            success: true,
            stream: updatedStream,
            message: 'Configurações de URLs salvas com sucesso'
        });

    } catch (error) {
        console.error('❌ [URLS] Erro ao salvar configurações:', error);
        ResponseHelper.error(res, 'Falha ao salvar configurações de URLs', 500);
    }
});
...
// REMOVIDO: Cleanup automático de usuários inativos
...
router.get('/streams/:id/online-users', async (req, res) => {
    ...
    try {
        ...
        if (onlineUsersInStream.length === 0) {
            ...
            const senderIds = giftSenders.map((s: any) => s._id);
            ...
            if (senderIds.length > 0) {
                ...
                const usersWithGiftData = senderUsers.map(u => {
                    ...
                    const senderData = giftSenders.find((s: any) => s._id === u.id);
...
// API para usuário entrar na live (GET version)
router.get('/streams/:streamId/join', async (req, res) => {
    try {
        const userId = await getUserIdFromToken(req);
        const streamId = req.params.streamId;

        console.log(`👤 [STREAM JOIN GET] Usuário ${userId} entrando na stream ${streamId}`);
        ...
        if (block) {
            return res.status(403).json({ success: false, error: 'Usuário bloqueado pelo host' });
        }
        ...
        console.log(`✅ [STREAM JOIN GET] Usuário ${userId} entrou na stream ${streamId}`);

        res.json({
            success: true,
            message: 'Usuário entrou na stream com sucesso',
            user: {
                id: user.id,
                name: user.name,
...
router.post('/streams/:id/join', async (req, res) => {
    ...
    try {
        ...
        // VERIFICACAO CRITICA: Se usuario ja esta online na mesma stream, nao fazer nada

        if (user.isOnline && user.currentStreamId === streamId) {

            console.warn(`🛑 USUARIO ${userId} JA ESTA ONLINE NA STREAM ${streamId} - IGNORANDO CHAMADA DUPLICADA`);

            return res.json({ success: true, message: 'Usuario ja esta online nesta stream', user });
...
router.post('/streams/:id/end-session', async (req, res) => {
    ...
    try {
        ...
        if (!stream) {
            ...
            console.warn(`⚠️ Stream ${streamId} não encontrada, mas continuando para limpar estado do usuário`);

            

            // Mesmo que a stream não exista, limpar o estado do usuário
            ...
            if (userId) {
                ...
                console.log(`✅ Estado do usuário ${userId} limpo mesmo sem stream encontrada`);

            }

            

            return res.json({ success: true, message: 'Stream não encontrada mas estado limpo' });
...
// API STARK - Iniciar live (padrão Buscast)
    ...
    try {
        ...
        if (!userId || !liveTitle || liveTitle.trim() === '') {
            ...
            return res.json({
                ...
                msg: 'userId e title/name são obrigatórios'
        ...
        // Buscar usuário para verificar existência
        ...
        const liveId = uuidv4(); // UUID único

        const streamId = `stream_${userId}_${uuidv4().substring(0, 8)}`; // streamId único
...
// Endpoint /api/streams/start - Cria registro provisório (isLive: false)
// Frontend chama antes de capturar mídia para obter streamKey e URLs
router.post('/streams/start', async (req, res) => {
    try {
        ...
        const user = await User.findOne({ id: userId });
        if (!user) {
            return res.status(404).json({ error: 'Usuário não encontrado', status: 'user_not_found' });
        }

        // Gerar streamKey única
        ...
        // Criar registro provisório — isLive: false até SRS on_publish
        ...
        console.log(`[STREAMS START] Stream provisório criado: ${streamKey} para usuário ${userId}`);
        ...
        console.error('[STREAMS START] Erro ao criar stream provisório:', error);
        res.status(500).json({
...
// Endpoint /api/lives/start - Porteiro oficial da transmissão
...
router.post('/lives/start', async (req, res) => {
    try {
        ...
        if (!streamId) {
            return res.status(400).json({ error: 'streamId é obrigatório', status: 'invalid_request' });
        }
        ...
        // Buscar stream existente por streamKey ou criar provisório
...
router.get('/lives/:id', async (req, res) => {
    ...
    try {
        ...
        // Transformar streamer para formato protegido usando mapper flexível
...
router.post('/lives/:id/end', async (req, res) => {
    ...
    try {
        ...
        console.log(`[STREAM-END] Encerrando stream: ${realId} por usuário: ${userId}`);
...
router.get('/live/nearby', async (req, res) => {
    ...
    try {
        ...
        // Retornar streams COM PROTEÇÃO DE DADOS SENSÍVEIS usando mapper flexível
...
// POST /api/streams/:streamId/end - Encerrar stream específica por ID
...
router.post('/streams/:streamId/end', async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        console.log(`[STREAM-END] Tentando encerrar stream: ${streamId} pelo usuário: ${userId}`);



        // Buscar stream específica
        ...
        // Verificar se o usuário é o dono da stream ou admin
        ...
        if (!isOwner && !isAdmin) {
            ...
            return res.status(403).json({ 
                ...
                message: 'Apenas o dono da stream ou administrador pode encerrá-la' 
        ...
        if (!stream.isLive) {
            ...
            return res.status(400).json({ 
                ...
                message: 'Stream já está encerrada' 
        ...
        // Atualizar status do usuário se não tiver outras streams ativas
...
// POST /api/streams/end-all - Encerrar todas as streams do usuário
...
router.post('/streams/end-all', async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        console.log(`[STREAM-END-ALL] Encerrando todas as streams do usuário: ${userId}`);



        // Buscar todas as streams ativas do usuário
        ...
        if (activeStreams.length === 0) {
            ...
            return res.status(400).json({ 
                ...
                message: 'Nenhuma stream ativa encontrada para este usuário' 
...
router.post('/streams/:streamId/heartbeat', async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        // Verificar se o usuário é o dono da stream
        ...
        // Se viewer count for enviado, atualizar também

        if (req.body.viewers !== undefined && typeof req.body.viewers === 'number') {

            updateData.viewers = Math.max(0, req.body.viewers); // Garantir número não negativo

        }



        // Se bandwidth for enviado, atualizar também (campo não existe no modelo, removido por enquanto)
        ...
        res.json({
            ...
            // bandwidth removido - campo não existe no modelo
...
// POST /api/admin/streams/:streamId/force-end - Forçar encerramento (admin)
    ...
    try {
        ...
        if (!user || user.level === undefined || user.level < 10) {
            ...
            return res.status(403).json({ 
                ...
                message: 'Acesso negado. Apenas administradores podem forçar encerramento' 
        ...
        console.log(`[ADMIN-FORCE-END] Admin ${userId} forçando encerramento da stream: ${streamId}`);
        ...
        await Streamer.findOneAndUpdate(
...

            {
                ...
                endReason: reason || 'Encerramento forçado por administrador',
        ...
        console.log(`[ADMIN-FORCE-END] Stream ${streamId} encerrada forçadamente pelo admin ${userId}`);
        ...
        res.json({
            ...
            message: 'Stream encerrada forçadamente com sucesso',
        ...
        console.error('[ADMIN-FORCE-END] Erro ao forçar encerramento:', error);

        res.status(500).json({ 

            success: false, 

            message: 'Erro interno ao forçar encerramento',
...
// GET /api/admin/streams/zombie-stats - Estatísticas de streams zumbis
...
router.get('/admin/streams/zombie-stats', async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        // Serviço de limpeza de streams zumbis removido

        res.json({

            success: true,

            message: 'Serviço de limpeza de streams zumbis foi desativado',
        ...
        console.error('[ADMIN-ZOMBIE-STATS] Erro ao obter estatísticas:', error);
...
router.post('/admin/streams/cleanup-zombies', async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        // Serviço de limpeza de streams zumbis foi removido
...
// POST /api/streams/prepare - Preparar live (cria registro mas não inicia transmissão)
    ...
    try {
        ...
        if (!liveTitle || liveTitle.trim() === '') {
            ...
            return res.status(400).json({ 
                ...
                message: 'Título da live é obrigatório (use "name" ou "title")' 
        ...
        console.log(`[STREAM-PREPARE] Live preparada: ${streamId} para usuário ${userId}`);
...
// POST /api/streams/:id/start - Iniciar transmissão (marcar como ativa)

router.post('/streams/:id/start', async (req, res) => {

    try {

        console.log('[STREAM-START] Iniciando transmissão...');
        ...
        console.log(`[STREAM-START] Stream ${id} iniciada para usuário ${userId}`);
