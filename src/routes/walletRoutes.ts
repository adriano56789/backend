...
router.get('/earnings/get/:id', async (req, res) => {
    try {
        ...
        const local_value = await convertBRL(brl_value, currency);
...
router.post('/earnings/calculate', async (req, res) => {
    try {
        ...
        const user = await User.findOne({ id: userId }).select('id country name');
        const currency = getCurrencyForCountry(user?.country);

        const multi = await calculateMultiCurrency(amount, brl_amount);
        ...
        res.json({
            ...
            local_net: local.net,
            breakdown: {
                conversion: `${amount} diamantes = ${formatCurrency(local.gross, currency)}`,
                fee: `Taxa de saque (20%): ${formatCurrency(local.fee, currency)}`,
                final: `Você receberia: ${formatCurrency(local.net, currency)}`
            }
        });
    } catch (error: any) {
...
router.get('/gifts/validate/:userId', async (req, res) => {
    try {
        ...
        const user = await User.findOne(
            { id: userId }
        ).select('id diamonds earnings withdrawal_method name email');
        if (!user) {
...
router.get('/user/data/:id', async (req, res) => {
    try {
        ...
        const user = await User.findOne(
            { id: userId }
        ).select('id diamonds earnings earnings_withdrawn diamonds_purchased withdrawal_method withdrawal_requests receptores enviados name email');
        if (!user) {
            return res.status(404).json({ error: 'Usuário não encontrado' });
        }
