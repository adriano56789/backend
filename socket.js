    ...
    io.on('connection', (socket) => {
        ...
        if (userId) {
            ...
            models_1.User.findOneAndUpdate({ id: userId }, { $set: { isOnline: true, lastSeen: now } }, { upsert: true }).then(() => {
                ...
                localIo.emit('user_status_changed', {
                    userId,
                    isOnline: true,
            ...
            if (data instanceof ArrayBuffer) {
                ...
                if (event.type === BinaryProtocol_1.EventType.GIFT_SENT && event.data) {
                    try {
                        ...
                        localIo.to(userRoom).emit('level_updated', {
                            userId: giftData.fromUserId,
                        ...
                        if (expResult.leveledUp) {
                            localIo.to(userRoom).emit('level_up', {
                                userId: giftData.fromUserId,
        ...
        socket.on('send_gift', async (data) => {
            ...
            try {
                ...
                if (data.fromUserId && totalValue > 0) {
                    ...
                    if (updatedSender) {
                        localIo.to(userRoom).emit('diamonds_updated', {
                            userId: data.fromUserId,
                        ...
                        localIo.to(userRoom).emit('user_stats_updated', {
                            userId: data.fromUserId,
                            stats: updatedSender
                        });
                    }
                    ...
                    if (data.streamId) {
                        ...
                        if (stream && stream.hostId && stream.hostId !== data.fromUserId) {
                            ...
                            if (updatedReceiver) {
                                localIo.to(`user_${stream.hostId}`).emit('diamonds_updated', {
                                ...
                                localIo.to(`user_${stream.hostId}`).emit('user_stats_updated', {
                                    userId: stream.hostId,
            ...
            if (buffer) {
                ...
                const totalValue = Math.max(1, Math.floor((data.giftPrice || 0) * (data.quantity || 1)));
                localIo.to(data.streamId).emit('live_gift_received', {
                    from: {
                        id: data.fromUserId || '',
                ...
                try {
                    ...
                    if (stream) {
                        localIo.to(data.streamId).emit('live_coins_updated', {
                            streamId: data.streamId,
        ...
        socket.on('follow_user', async (data) => {
            try {
                ...
                localIo.emit('follow_notification', {
                    type: 'follow',
                    followerId: data.followerId,
        ...
        socket.on('friendship_request', async (data) => {
            try {
                ...
                localIo.to(data.toUserId).emit('friendship_notification', {
        ...
        socket.on('live_broadcast', async (data) => {
            try {
                ...
                localIo.emit('live_state_changed', broadcastData);
        ...
        socket.on('friendship_accept', async (data) => {
            try {
                ...
                localIo.emit('friendship_notification', {
